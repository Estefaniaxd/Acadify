...
class AvatarAsset(Base):
    ...
    display_name = Column(
        String(100),
        nullable=True,
        doc="Nombre amigable para mostrar en UI (opcional)"
    )
    ...
    width = Column(
        Integer,
        nullable=False,
        doc="Ancho de la imagen en píxeles"
    )
    
    height = Column(
        Integer,
        nullable=False,
        doc="Alto de la imagen en píxeles"
    )
    
    meta_info = Column(
        JSON,
        nullable=True,
        doc="Metadatos adicionales: color, tags, rarity, etc."
    )
