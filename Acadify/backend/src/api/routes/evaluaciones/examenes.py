...
from src.schemas.evaluaciones import (
    ExamenCreate, ExamenUpdate, ExamenResponse, ExamenCompleto, ExamenParaEstudiante,
    PreguntaExamenCreate, PreguntaExamenResponse
)
...
def crear_examen(
    *,
    ...
    # Crear el examen
    created_exam = examen.create(db=db, obj_in=exam_in)
    return created_exam
...
def listar_examenes(
    ...
    estado: Optional[EstadoExamen] = Query(None, description="Filtrar por estado"),
    tipo_examen: Optional[TipoExamen] = Query(None, description="Filtrar por tipo"),
    curso_id: Optional[str] = Query(None, description="Filtrar por curso"),
    busqueda: Optional[str] = Query(None, description="Búsqueda por título o descripción")
):
    """
...
def actualizar_examen(
    *,
    ...
    updated_exam = examen.update(db=db, db_obj=exam, obj_in=exam_in)
    return updated_exam
...
def duplicar_examen(
    *,
    ...
    nuevo_titulo: str = Body(..., embed=True, description="Título para el examen duplicado"),
    current_user: Usuario = Depends(get_current_user)
):
    """
...
def obtener_estadisticas_profesor(
    *,
    ...
    profesor_id = current_user.user_id
    estadisticas = examen.get_estadisticas_profesor(db=db, profesor_id=profesor_id)
...
def obtener_examenes_proximos(
    *,
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(get_current_user),
    dias_adelante: int = Query(7, ge=1, le=30, description="Días hacia adelante para buscar")
):
    """
    ...
    examenes_proximos = examen.get_examenes_proximos(
        db=db, 
        profesor_id=current_user.user_id, 
        dias_adelante=dias_adelante
    )
    
    return examenes_proximos