...
from ...schemas.academic.material_educativo import (
    MaterialEducativo, MaterialEducativoCreate, MaterialEducativoUpdate, 
    MaterialEducativoDetallado, MaterialEducativoSubirVersion, FiltrosMaterial,
    EstadisticasMaterial, SincronizacionDrive
)
from ...crud.academic.crud_material_educativo import material_educativo_crud
from ...core.redis_manager import redis_manager
from ...models.users.usuario import Usuario
from ...enums.users.usuario_enums import TipoUsuario
from ...enums.academic.material_educativo_enums import CarpetaMaterial, TipoMaterialEducativo, EstadoMaterial
    ...
    *,
    ...
    try:
        ...
        material_data = {
            ...
            "autor_id": str(nuevo_material.autor_id) if nuevo_material.autor_id else None,
        ...
        raise HTTPException(
            ...
            detail=f"Error creando material: {str(e)}"
        )
...
async def obtener_material(
    ...
    cached_data = redis_manager.obtener_material_cache(str(material_id))
    ...
    if material.estado != EstadoMaterial.activo and material.autor_id != current_user.usuario_id:
        if current_user.tipo_usuario not in [TipoUsuario.coordinador, TipoUsuario.admin]:
            raise HTTPException(
...
async def actualizar_material(
    *,
    ...
    if material.autor_id != current_user.usuario_id and current_user.tipo_usuario != TipoUsuario.coordinador:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def eliminar_material(
    ...
    material_archivado = material_educativo_crud.update(
        db=db,
        db_obj=material,
...
async def subir_nueva_version(
    *,
    ...
    # Verificar permisos
    if material_original.autor_id != current_user.usuario_id and current_user.tipo_usuario != TipoUsuario.coordinador:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Error creando nueva versión: {str(e)}"
        )
...
async def buscar_material(
    ...
    materiales = material_educativo_crud.buscar_material(
        db=db,
...
async def obtener_material_popular(
    ...
    materiales = material_educativo_crud.get_material_popular(
        db=db,
        limit=limit,
        carpeta=carpeta
    )
...
async def obtener_estadisticas_material(
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(require_roles([TipoUsuario.coordinador, TipoUsuario.admin]))
):
    """Obtener estadísticas generales del material educativo"""
    
    estadisticas = material_educativo_crud.get_estadisticas_material(db=db)
...
async def obtener_versiones_material(
    ...
    if material.autor_id != current_user.usuario_id and current_user.tipo_usuario not in [TipoUsuario.coordinador, TipoUsuario.admin]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="No tienes permisos para ver las versiones de este material"
        )
    
    versiones = material_educativo_crud.get_versiones_material(db=db, material_id=material_id)
