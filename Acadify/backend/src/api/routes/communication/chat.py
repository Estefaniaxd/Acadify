...
from crud.communication.chat import (
    ...
    crud_lectura_mensaje, crud_notificacion, crud_config_notificaciones
)
from schemas.communication.chat_schemas import (
    # Salas
    SalaChatCreate, SalaChatUpdate, SalaChatResponse, SalaChatDetallada,
    # Participantes
    ParticipanteSalaCreate, ParticipanteSalaUpdate, ParticipanteSalaResponse,
    # Mensajes
    MensajeCreate, MensajeUpdate, MensajeResponse, MensajeDetallado, ReaccionMensaje,
    # Lecturas
    MarcarLectura, LecturaMensajeResponse,
    # Notificaciones
    NotificacionCreate, NotificacionResponse, MarcarNotificacionLeida,
    # Configuración
    ConfiguracionNotificacionesUpdate, ConfiguracionNotificacionesResponse,
    # Filtros
    FiltrosSalas, FiltrosMensajes, FiltrosNotificaciones,
    # Estadísticas
    EstadisticasSala, EstadisticasUsuario
)
...
async def websocket_chat(
    ...
    sala_id: Optional[str] = Query(None),
...
async def crear_sala(
    *,
    ...
    sala = crud_sala_chat.create_with_creator(
        db=db, 
        obj_in=sala_in, 
        creador_id=str(current_user.id)
    )
...
@router.get("/salas/{sala_id}/participantes", response_model=List[ParticipanteSalaResponse])
...
async def agregar_participante(
    ...
    *,
    ...
    if not mi_participacion or not (mi_participacion.es_admin or mi_participacion.es_moderador):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
@router.put("/salas/{sala_id}/participantes/{usuario_id}", response_model=ParticipanteSalaResponse)
async def actualizar_participante(
    ...
    *,
    ...
    participante = crud_participante_sala.update(db=db, db_obj=participante, obj_in=participante_in)
    return ParticipanteSalaResponse.from_orm(participante)
...
async def remover_participante(
    ...
    puede_remover = (
        mi_participacion and mi_participacion.es_admin
    ) or str(current_user.id) == str(usuario_id)
...
async def enviar_mensaje(
    *,
    ...
    participante = crud_participante_sala.get_participante(
        db=db,
        sala_id=str(mensaje_in.sala_id),
        usuario_id=str(current_user.id)
    )
...
@router.get("/configuracion/notificaciones", response_model=ConfiguracionNotificacionesResponse)
...
@router.put("/configuracion/notificaciones", response_model=ConfiguracionNotificacionesResponse)
