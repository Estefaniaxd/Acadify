...
from src.models.evaluaciones import (
    PreguntaExamen, RespuestaEstudiante, TipoPregunta, DificultadPregunta
)
...
class CalificadorAutomatico:
    ...
    def calificar_respuesta(
        ...
        resultado["puntuacion_obtenida"] = max(0, min(resultado["puntuacion_obtenida"], pregunta.puntuacion))
        
        return resultado
    
    def _calificar_opcion_multiple(self, pregunta: PreguntaExamen, respuesta: RespuestaEstudiante) -> Dict[str, Any]:
        ...
        if isinstance(respuesta_correcta.get("respuestas_correctas"), list):
            ...
            if opcion_correcta == opcion_seleccionada:
                ...
                resultado["feedback_automatico"] = f"Incorrecto. La respuesta correcta era: {opcion_correcta}"
    ...
    def _calificar_multiple_correctas(
        ...
        if correctas == seleccionadas:
            ...
            resultado["puntuacion_obtenida"] = pregunta.puntuacion
            resultado["es_correcta"] = True
            resultado["feedback_automatico"] = "¡Perfecto! Seleccionaste todas las opciones correctas."
            ...
            resultado["es_correcta"] = len(interseccion) == len(correctas) and len(seleccionadas) == len(correctas)
            
            # Feedback detallado
            feedback_parts = []
            if interseccion:
                feedback_parts.append(f"Acertaste {len(interseccion)} de {len(correctas)} opciones correctas.")
            if seleccionadas - correctas:
                feedback_parts.append(f"Seleccionaste {len(seleccionadas - correctas)} opciones incorrectas.")
            if correctas - seleccionadas:
                feedback_parts.append(f"Te faltaron {len(correctas - seleccionadas)} opciones correctas.")
            
            resultado["feedback_automatico"] = " ".join(feedback_parts)
            
        else:
            resultado["feedback_automatico"] = "Incorrecto. Debes seleccionar todas las opciones correctas."
        
        return resultado
    
    def _calificar_verdadero_falso(self, pregunta: PreguntaExamen, respuesta: RespuestaEstudiante) -> Dict[str, Any]:
        ...
        if respuesta_correcta == respuesta_estudiante:
            ...
            resultado["feedback_automatico"] = f"Incorrecto. La respuesta correcta era: {'Verdadero' if respuesta_correcta else 'Falso'}"
    ...
    def _calificar_completar(self, pregunta: PreguntaExamen, respuesta: RespuestaEstudiante) -> Dict[str, Any]:
        ...
        for hueco, respuesta_correcta in respuestas_correctas.items():
            ...
            opciones_correctas = [opt.strip() for opt in str(respuesta_correcta).split("|")]
        ...
        if pregunta.puntos_respuesta_parcial and total_huecos > 0:
            ...
            resultado["feedback_automatico"] = f"Completaste correctamente {huecos_correctos} de {total_huecos} espacios."
        else:
            if huecos_correctos == total_huecos:
                resultado["puntuacion_obtenida"] = pregunta.puntuacion
                resultado["es_correcta"] = True
                resultado["feedback_automatico"] = "¡Perfecto! Completaste todos los espacios correctamente."
            else:
                resultado["feedback_automatico"] = f"Incorrecto. Completaste {huecos_correctos} de {total_huecos} espacios correctamente."
        
        return resultado
    
    def _calificar_emparejamiento(self, pregunta: PreguntaExamen, respuesta: RespuestaEstudiante) -> Dict[str, Any]:
        ...
        if pregunta.puntos_respuesta_parcial and total_emparejamientos > 0:
            ...
            resultado["puntuacion_obtenida"] = pregunta.puntuacion * proporcion
            resultado["es_correcta"] = emparejamientos_correctos_count == total_emparejamientos
            resultado["feedback_automatico"] = f"Emparejaste correctamente {emparejamientos_correctos_count} de {total_emparejamientos} elementos."
        else:
            if emparejamientos_correctos_count == total_emparejamientos:
                resultado["puntuacion_obtenida"] = pregunta.puntuacion
                resultado["es_correcta"] = True
                resultado["feedback_automatico"] = "¡Perfecto! Todos los emparejamientos son correctos."
            else:
                resultado["feedback_automatico"] = f"Incorrecto. {emparejamientos_correctos_count} de {total_emparejamientos} emparejamientos son correctos."
        
        return resultado
    
    def _calificar_ordenamiento(self, pregunta: PreguntaExamen, respuesta: RespuestaEstudiante) -> Dict[str, Any]:
        ...
        if orden_correcto == orden_estudiante:
            resultado["puntuacion_obtenida"] = pregunta.puntuacion
            resultado["es_correcta"] = True
            resultado["feedback_automatico"] = "¡Perfecto! El orden es completamente correcto."
            ...
            resultado["detalles_calificacion"]["posiciones_correctas"] = posiciones_correctas_lista
            
            if total_items > 0:
                proporcion = posiciones_correctas / total_items
                resultado["puntuacion_obtenida"] = pregunta.puntuacion * proporcion
                resultado["es_correcta"] = posiciones_correctas == total_items
                resultado["feedback_automatico"] = f"Ordenaste correctamente {posiciones_correctas} de {total_items} elementos en sus posiciones correctas."
            
        else:
            resultado["feedback_automatico"] = "Incorrecto. El orden no es completamente correcto."
        
        return resultado
    
    def _calificar_ensayo_keywords(self, pregunta: PreguntaExamen, respuesta: RespuestaEstudiante) -> Dict[str, Any]:
        ...
        if not palabras_clave:
            ...
            resultado["feedback_automatico"] = "No hay criterios de evaluación específicos configurados"
            return resultado
        ...
        total_peso = sum(info.get("peso", 1.0) if isinstance(info, dict) else 1.0 for info in palabras_clave)
        ...
        longitud_minima = pregunta.configuracion_avanzada.get("longitud_minima", 50) if pregunta.configuracion_avanzada else 50
        ...
        puntuacion_final = pregunta.puntuacion * (
            (proporcion_palabras * peso_palabras) + 
            (factor_longitud * peso_longitud)
        )
        
        resultado["puntuacion_obtenida"] = round(puntuacion_final, 2)
        resultado["es_correcta"] = resultado["puntuacion_obtenida"] >= (pregunta.puntuacion * 0.6)
        ...
        resultado["feedback_automatico"] = f"Se encontraron {len(palabras_encontradas)} de {len(palabras_clave)} conceptos clave esperados. " \
                                         f"Longitud del texto: {longitud_texto} caracteres."
        
        return resultado
    
    def _calificar_ensayo_similitud(self, pregunta: PreguntaExamen, respuesta: RespuestaEstudiante) -> Dict[str, Any]:
        ...
        if pregunta.respuesta_correcta and isinstance(pregunta.respuesta_correcta, dict):
        ...
        similitud = SequenceMatcher(None, 
                                   ...
                                   respuesta_modelo.lower().strip()).ratio()
        
        # Ajustar puntuación basada en similitud
        puntuacion_base = pregunta.puntuacion * similitud
        ...
        resultado["feedback_automatico"] = f"Similitud con respuesta modelo: {round(similitud * 100, 1)}%. " \
                                         "Nota: Esta es una evaluación automática preliminar."
        
        return resultado
    
    def _calificar_ensayo_combinado(self, pregunta: PreguntaExamen, respuesta: RespuestaEstudiante) -> Dict[str, Any]:
        ...
        if "similitud_texto" in resultado_similitud["detalles_calificacion"]:
            ...
            feedback_parts.append(f"• Similitud con modelo: {round(similitud * 100, 1)}%")
        
        feedback_parts.append("IMPORTANTE: Esta es una calificación automática preliminar que requiere revisión manual.")
    ...
    def _extraer_palabras_clave(self, pregunta: PreguntaExamen) -> List[Any]:
        ...
        if not palabras_clave and pregunta.explicacion:
            ...
            palabras_importantes = self._extraer_palabras_importantes(pregunta.explicacion)
    ...
    def _extraer_palabras_importantes(self, texto: str) -> List[str]:
        ...
        stop_words = {
            ...
            'son', 'con', 'para', 'al', 'del', 'los', 'las', 'una', 'sobre', 'todo', 'también', 'después', 'muy',
            'sin', 'como', 'entre', 'durante', 'tanto', 'antes', 'bajo', 'hasta', 'desde', 'cuando', 'donde',
            'este', 'esta', 'estos', 'estas', 'ese', 'esa', 'esos', 'esas', 'aquel', 'aquella', 'aquellos',
            'aquellas', 'mi', 'tu', 'su', 'nuestro', 'vuestro', 'mio', 'tuyo', 'suyo', 'nuestros', 'vuestros'
        }
        ...
        palabras_importantes = sorted(frecuencias.keys(), key=lambda k: frecuencias[k], reverse=True)
    ...
    def generar_reporte_calificacion(
        ...
        reporte = {
            ...
            "porcentaje": round((resultado_calificacion["puntuacion_obtenida"] / pregunta.puntuacion) * 100, 2),
            "es_correcta": resultado_calificacion["es_correcta"],
            "confianza_calificacion": resultado_calificacion["confianza_calificacion"],
            "requiere_revision_manual": resultado_calificacion["confianza_calificacion"] < 0.8,
        ...
        if pregunta.tipo_pregunta in [TipoPregunta.ENSAYO, TipoPregunta.RESPUESTA_CORTA]:
            reporte["recomendaciones"].append("Se recomienda revisión manual para preguntas de texto libre")
            
            if resultado_calificacion["confianza_calificacion"] < 0.6:
                reporte["recomendaciones"].append("Baja confianza en calificación automática - revisión manual necesaria")
        
        if not resultado_calificacion["es_correcta"] and pregunta.tipo_pregunta in [TipoPregunta.OPCION_MULTIPLE, TipoPregunta.VERDADERO_FALSO]:
            reporte["recomendaciones"].append("Considerar proporcionar retroalimentación adicional al estudiante")
