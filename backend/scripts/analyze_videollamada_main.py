...
bd_cols = inspector.get_columns("videollamadas")

print("="*100)
print("📋 ANÁLISIS: Videollamada (Clase Principal)")
print("="*100)
print()
...
# Extraer solo Videollamada (termina cuando empieza VideollamadaParticipante)
pattern = r"class Videollamada\(Base\):.*?(?=class VideollamadaParticipante\(Base\):)"
match = re.search(pattern, content, re.DOTALL)

if match:
    videollamada_content = match.group(0)
    model_cols = set(re.findall(r"(\w+)\s*=\s*Column\(", videollamada_content))
    ...
    print("❌ No se encontró Videollamada")
