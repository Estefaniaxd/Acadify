"""
Análisis detallado de discrepancias en InventarioUsuario
"""
import asyncio
import asyncpg
...
async def main():
    print("\n" + "="*70)
    print("🔍 ANÁLISIS: InventarioUsuario - Modelo vs Base de Datos")
    print("="*70 + "\n")
    ...
    try:
        ...
        mapper = inspect(InventarioUsuario)
        ...
        if solo_bd:
            print("\n🔧 Agregar al modelo InventarioUsuario:\n")
            for col_name in sorted(solo_bd):
                col_info = next(c for c in bd_columns if c['column_name'] == col_name)
