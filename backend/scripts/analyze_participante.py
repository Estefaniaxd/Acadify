...
bd_cols = inspector.get_columns("videollamada_participantes")

print("="*100)
print("📋 ANÁLISIS: VideollamadaParticipante")
print("="*100)
print()
...
pattern = r"class VideollamadaParticipante\(Base\):.*?(?=class \w+\(Base\):|$)"
match = re.search(pattern, content, re.DOTALL)

if match:
    participante_content = match.group(0)
    model_cols = set(re.findall(r"(\w+)\s*=\s*Column\(", participante_content))
    ...
    print("❌ No se encontró VideollamadaParticipante")
