...
class LocalStorageBackend(StorageBackend):
    ...
    async def save_image(self, image: Image.Image, path: str) -> str:
        ...
        await asyncio.get_event_loop().run_in_executor(
            None, lambda: image.save(full_path, "PNG", optimize=True)
        )
    ...
    def get_public_url(self, path: str) -> str:
        ...
        if not path.startswith("/static/"):
            path = "/" + path if path.startswith("static/") else "/static/" + path
...
class S3StorageBackend(StorageBackend):
    ...
    def __init__(self, bucket_name: str, region: str = "us-east-1") -> None:
        ...
        msg = "S3 backend not implemented yet"
...
class StorageManager:
    ...
    async def save_avatar_image(
        self, image: Image.Image, user_id: str, filename: str
    ) -> str:
    ...
    async def save_preview_image(self, image: Image.Image, filename: str) -> str:
    ...
    async def save_profile_photo(
        ...
        """Guarda foto de perfil custom del usuario.

        Args:
            image: Imagen PIL a guardar
            user_id: ID del usuario
            filename: Nombre del archivo (ej: "profile_123.jpg")

        Returns:
            URL pública de la imagen
        """
        path = f"profile_photos/{user_id}/{filename}"
        return await self.backend.save_image(image, path)

    async def save_banner(self, image: Image.Image, user_id: str, filename: str) -> str:
        """Guarda banner/portada custom del usuario.

        Args:
            image: Imagen PIL a guardar
            user_id: ID del usuario
            filename: Nombre del archivo (ej: "banner_123.jpg")

        Returns:
            URL pública de la imagen
        """
        path = f"banners/{user_id}/{filename}"
        return await self.backend.save_image(image, path)

    async def delete_profile_photo(self, user_id: str, filename: str) -> bool:
        """Elimina foto de perfil custom.
    ...
    async def delete_banner(self, user_id: str, filename: str) -> bool:
        """Elimina banner custom.
    ...
    def get_storage_info(self) -> dict[str, Any]:
        ...
        info = {"type": backend_type, "backend_class": self._backend.__class__.__name__}
