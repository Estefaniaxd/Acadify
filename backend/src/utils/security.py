...
class LoginAttemptManager:
    ...
    async def record_failed_attempt(self, identifier: str) -> Dict[str, Any]:
        ...
        if attempts >= self.max_attempts:
            lockout_until = datetime.now(timezone.utc) + timedelta(minutes=self.lockout_duration)
...
def is_strong_password(password: str) -> tuple[bool, str]:
    ...
    if not all([has_upper, has_lower, has_digit, has_special]):
        return False, "La contraseña debe contener al menos: 1 mayúscula, 1 minúscula, 1 número y 1 carácter especial"
