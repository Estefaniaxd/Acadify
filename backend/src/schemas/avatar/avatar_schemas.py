...
class LayerItem(BaseModel):
    ...
    file: str = Field(
        ..., 
        description="Ruta relativa del archivo (ej: hair/short_black.png)",
        min_length=1,
        max_length=255
    )
    ...
    def validate_category(cls, v):
        ...
        allowed_categories = {
            'base', 'hair', 'eyes', 'clothes', 'accessories', 'backgrounds'
        }
    ...
    def validate_file(cls, v):
        ...
        if '/' not in v:
            raise ValueError('File must include category path (e.g., hair/short_black.png)')
        return v
...
class PreviewRequest(BaseModel):
    ...
    layers: List[LayerItem] = Field(
        ..., 
        description="Lista de capas del avatar",
        min_length=1,
        max_length=10
    )
...
class SaveAvatarRequest(BaseModel):
    ...
    is_active: bool = Field(
        default=False, 
        description="Si este avatar será el activo del usuario"
    )
    is_public: bool = Field(
        default=True, 
        description="Si el avatar será público"
    )
...
class UpdateAvatarRequest(BaseModel):
    ...
    is_active: Optional[bool] = Field(
        default=None,
        description="Si este avatar será el activo"
    )
...
class AssetInfo(BaseModel):
    ...
    meta_info: Dict[str, Any] = Field(default_factory=dict, description="Metadatos adicionales")


class ManifestResponse(BaseModel):
    """Response del manifest de assets."""
    resolution: List[int] = Field(..., description="Resolución estándar [width, height]")
    categories: Dict[str, List[AssetInfo]] = Field(..., description="Assets por categoría")
...
class UserAvatarListResponse(BaseModel):
    ...
    active_avatar_id: Optional[str] = Field(default=None, description="ID del avatar activo")
...
class AvatarStatsResponse(BaseModel):
    ...
    total_users_with_avatars: int = Field(..., description="Usuarios con al menos un avatar")
    popular_avatars: List[PopularAvatarResponse] = Field(..., description="Avatars más populares")
    categories_stats: Dict[str, int] = Field(..., description="Estadísticas por categoría")
