...
class LoginRequest(BaseModel):
    ...
    otp_code: str | None = Field(
        None, description="Código TOTP para 2FA (si está habilitado)"
    )
...
class PasswordResetRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"correo_institucional": EXAMPLE_CORREO}}
...
class PasswordResetConfirm(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"token": "abc123def456", "new_password": "NuevaContraseña123!"}
        }
...
class TwoFAVerifyRequest(BaseModel):
    ...
    otp_code: str = Field(
        ..., min_length=6, max_length=6, description="Código TOTP de 6 dígitos"
    )
...
class LoginAttemptResponse(BaseModel):
    ...
    lockout_ends_at: datetime | None = Field(
        None, description="Cuando termina el bloqueo"
    )
