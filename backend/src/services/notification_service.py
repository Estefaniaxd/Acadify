...
from crud.communication.chat import crud_config_notificaciones, crud_notificacion
from db.session import SessionLocal
from models.academic.tarea import EstadoTarea, Tarea
from models.communication.chat import ConfiguracionNotificaciones, Notificacion
...
celery_app = Celery(
    "acadify_notifications", broker=settings.REDIS_URL, backend=settings.REDIS_URL
)
        ...
        try:
            ...
            for email_data in emails:
                try:
                    ...
                    server.sendmail(
                        self.from_email, email_data["to_email"], msg.as_string()
                    )
                    results["sent"] += 1

                except Exception as e:
                    logger.exception(f"Error enviando email individual: {e}")
                    results["failed"] += 1

            server.quit()

        except Exception as e:
            logger.exception(f"Error en envío masivo: {e}")
...
class NotificationService:
    ...
    def should_send_email_notification(
        self, db: Session, usuario_id: str, tipo_notificacion: str
    ) -> bool:
        """Verificar si debe enviar notificación por email según configuración."""
        config = crud_config_notificaciones.get_by_usuario(db=db, usuario_id=usuario_id)
        ...
        dia_actual = datetime.now(UTC).weekday() + 1  # Python: 0=Lun, 6=Dom
        ...
        if tipo_notificacion == "tarea_vencimiento_1h":
            return config.tareas_vencimiento_1h and config.urgentes_email
        if tipo_notificacion == "mencion":
            return config.menciones and config.menciones_email
        if tipo_notificacion == "tarea_nueva":
            return config.tareas_nuevas
        if tipo_notificacion == "tarea_calificada":
            return config.tareas_calificadas
        if tipo_notificacion == "mensaje_directo":
    ...
    def render_email_template(
        ...
        subject_template = Template(
            template_data.get("subject", "Notificación de Acadify")
        )
...
def check_task_deadlines(self):
    ...
    try:
            ...
            for estudiante in estudiantes:
                ...
                if notification_service.should_send_email_notification(
                    ...
                    subject, html_content, text_content = (
                        notification_service.render_email_template(
        ...
        logger.info(
            f"Check deadlines 24h: {emails_sent} emails enviados, {notifications_created} notificaciones creadas"
        )
        return {
        ...
        logger.exception(f"Error en check_task_deadlines: {e}")
        db.rollback()
        db.close()
        raise self.retry(countdown=300, exc=e) from e  # Reintentar en 5 minutos
...
def check_urgent_deadlines(self):
    ...
    try:
        ...
        for tarea in tareas:
            ...
            for estudiante in estudiantes:
                ...
                notification_service.create_notification(
                    db=db,
                    usuario_id=str(estudiante.id),
                    titulo="🚨 URGENTE: Tarea vence en 1 hora",
                    mensaje=f"Tu tarea '{tarea.titulo}' vence el {tarea.fecha_limite.strftime('%d/%m/%Y a las %H:%M')}",
        ...
        logger.info(
            f"Check urgent deadlines: {emails_sent} emails enviados, {notifications_created} notificaciones creadas"
        )
        return {
            "emails_sent": emails_sent,
            "notifications_created": notifications_created,
        ...
        logger.exception(f"Error en check_urgent_deadlines: {e}")
        db.rollback()
        db.close()
        raise self.retry(countdown=60, exc=e) from e  # Reintentar en 1 minuto
...
def send_daily_summary(self):
    ...
    try:
        ...
        configs = (
            ...
            .filter(ConfiguracionNotificaciones.resumen_diario_email)
            .all()
        )
        ...
        for config in configs:
            try:
                ...
                logger.exception(
                    f"Error enviando resumen diario a usuario {config.usuario_id}: {e}"
                )
                continue
        ...
        logger.exception(f"Error en send_daily_summary: {e}")
        return {"error": str(e)}
...
def cleanup_old_notifications():
    ...
    try:
        ...
        deleted_count = (
            ...
            .filter(Notificacion.leida, Notificacion.fecha_creacion < cutoff_date)
        ...
        logger.exception(f"Error en cleanup_old_notifications: {e}")
        return {"error": str(e)}


@celery_app.task
def send_mention_notification(usuario_id: str, mensaje_id: str, sala_id: str) -> None:
    ...
    try:
        ...
        logger.exception(f"Error enviando notificación de mención: {e}")
...
def send_test_email(to_email: str) -> bool:
    ...
    """.format(
        datetime.now(UTC).strftime("%d/%m/%Y %H:%M")
    )
