...
from src.models.evaluaciones import (
    ...
    BancoPregunta, EventoAntiTrampa, EstadisticaExamen,
    TipoExamen, EstadoExamen, TipoPregunta, EstadoIntento
)
...
class ServicioIntegracionExamenes:
    ...
    def procesar_intento_completo(
        ...
        eventos_finales: Optional[List[Dict[str, Any]]] = None
    ) -> Dict[str, Any]:
        """
        ...
        preguntas = db.query(PreguntaExamen).filter(
            PreguntaExamen.examen_id == intento.examen_id
        ...
        try:
            ...
            reporte_integridad = self.detector_trampa.generar_reporte_integridad(db, intento_id)
            ...
            db.rollback()
            return {
                "error": f"Error procesando intento: {str(e)}",
                "intento_id": intento_id
            }
    ...
    def crear_examen_desde_banco(
        ...
        try:
            ...
            if not preguntas_seleccionadas:
                return {"error": "No se encontraron preguntas que cumplan los criterios"}
            ...
            nuevo_examen = Examen(
                ...
                tipo_examen=TipoExamen(configuracion_examen.get("tipo_examen", "EVALUACION")),
                ...
                mostrar_resultados_inmediatos=configuracion_examen.get("mostrar_resultados", True),
            ...
            for i, pregunta_banco in enumerate(preguntas_seleccionadas):
                pregunta_examen = PreguntaExamen(
                    ...
                    puntos_respuesta_parcial=configuracion_examen.get("puntos_parciales", True),
            ...
            reporte_creacion = {
                ...
                "preguntas_por_tipo": self._contar_preguntas_por_tipo(preguntas_seleccionadas),
                "preguntas_por_dificultad": self._contar_preguntas_por_dificultad(preguntas_seleccionadas),
                "tiempo_estimado_total": sum(p.tiempo_estimado or 0 for p in preguntas_seleccionadas),
            ...
            db.rollback()
            return {"error": f"Error creando examen: {str(e)}"}
    ...
    def clonar_examen(
        ...
        try:
            ...
            examen_original = db.query(Examen).filter(
            ...
            preguntas_originales = db.query(PreguntaExamen).filter(
            ...
            db.rollback()
            return {"error": f"Error clonando examen: {str(e)}"}
        ...
        comparacion = {
            ...
            "resumen_comparativo": {
                ...
                "mejor_tasa_aprobacion": max(
                    e["metricas_generales"]["tasa_aprobacion"] 
                    for e in examenes_data
                ),
                ...
                "mayor_participacion": max(
                    e["metadata"]["total_intentos_analizados"] 
                    for e in examenes_data
                )
            },
    ...
    def ejecutar_mantenimiento_sistema(
        ...
        try:
            ...
            db.rollback()
            resultados_mantenimiento["errores"].append(f"Error en mantenimiento: {str(e)}")
    ...
    def _calificar_intento_completo(
        ...
        for i, (pregunta, respuesta_data) in enumerate(zip(preguntas, respuestas_datos)):
            ...
            if respuesta_existente:
                ...
                # Actualizar datos
                respuesta.respuesta_estudiante = respuesta_data.get("respuesta_estudiante")
                ...
                respuesta = RespuestaEstudiante(
                    ...
                    respuesta_estudiante=respuesta_data.get("respuesta_estudiante"),
            ...
            respuesta.puntuacion_obtenida = resultado_calificacion["puntuacion_obtenida"]
            respuesta.feedback_automatico = resultado_calificacion["feedback_automatico"]
            respuesta.detalles_calificacion = resultado_calificacion["detalles_calificacion"]
            
            # Acumular puntuación
            resultados_calificacion["puntuacion_total"] += resultado_calificacion["puntuacion_obtenida"]
        ...
        if resultados_calificacion["puntuacion_maxima"] > 0:
            resultados_calificacion["porcentaje"] = round(
                (resultados_calificacion["puntuacion_total"] / resultados_calificacion["puntuacion_maxima"]) * 100, 2
            )
        else:
    ...
    def _finalizar_intento(
        ...
        intento.datos_adicionales.update({
            ...
            "puntuacion_integridad": reporte_integridad.get("metricas_generales", {}).get("puntuacion_integridad"),
            "eventos_criticos": reporte_integridad.get("metricas_generales", {}).get("eventos_criticos"),
            "calificacion_automatica": {
                "porcentaje": resultado_calificacion["porcentaje"],
                "preguntas_correctas": resultado_calificacion["resumen"]["correctas"],
                "preguntas_parciales": resultado_calificacion["resumen"]["parciales"]
            }
        })
    ...
    def _generar_alertas_recomendaciones(
        ...
        if "metricas_generales" in estadisticas_examen:
            promedio_examen = estadisticas_examen["metricas_generales"].get("promedio_calificaciones", 0)
        ...
        preguntas_baja_confianza = [
            p for p in resultado_calificacion["preguntas_calificadas"]
    ...
    def _seleccionar_preguntas_banco(
        ...
        if "dificultades" in criterios:
            query = query.filter(BancoPregunta.dificultad.in_(criterios["dificultades"]))
        
        if "tipos_pregunta" in criterios:
            query = query.filter(BancoPregunta.tipo_pregunta.in_(criterios["tipos_pregunta"]))
        
        if "puntuacion_minima" in criterios:
            query = query.filter(BancoPregunta.puntuacion_sugerida >= criterios["puntuacion_minima"])
    ...
    def _contar_preguntas_por_tipo(self, preguntas: List[BancoPregunta]) -> Dict[str, int]:
    ...
    def _contar_preguntas_por_dificultad(self, preguntas: List[BancoPregunta]) -> Dict[str, int]:
    ...
    def _requiere_notificacion(self, resultado_procesamiento: Dict[str, Any]) -> bool:
        ...
        alertas_altas = [
            a for a in resultado_procesamiento.get("alertas", [])
            if a.get("prioridad") == "alta"
        ]
        
        return len(alertas_altas) > 0
    
    def _programar_notificacion(self, db: Session, resultado: Dict[str, Any]):
    ...
    def _limpiar_intentos_abandonados(self, db: Session) -> Dict[str, Any]:
        ...
        intentos_abandonados = db.query(IntentoExamen).filter(
            and_(
    ...
    def _actualizar_estadisticas_desactualizadas(self, db: Session) -> Dict[str, Any]:
        ...
        examenes_actualizar = db.query(Examen).filter(
            Examen.estado_examen == EstadoExamen.ACTIVO
        ).limit(50).all()  # Limitar para no sobrecargar
    ...
    def _revisar_eventos_anti_trampa(self, db: Session) -> Dict[str, Any]:
        ...
        eventos_antiguos = db.query(EventoAntiTrampa).filter(
            EventoAntiTrampa.timestamp < limite_antiguedad
        ).count()
    ...
    def _optimizar_banco_preguntas(self, db: Session) -> Dict[str, Any]:
        ...
        preguntas_sin_usar = db.query(BancoPregunta).filter(
            BancoPregunta.veces_utilizada == 0
        ).count()
