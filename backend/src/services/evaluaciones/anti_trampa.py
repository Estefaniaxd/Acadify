...
from src.models.evaluaciones import EventoAntiTrampa, Examen, IntentoExamen, TipoEvento
...
class DetectorAntiTrampa:
    ...
    def analizar_patron_actividad(
        self, db: Session, intento_id: int, ventana_tiempo: int = 300  # 5 minutos
    ) -> dict[str, Any]:
        """Analizar patrones de actividad sospechosa."""
        timestamp_limite = datetime.now(UTC) - timedelta(seconds=ventana_tiempo)
        ...
        eventos = (
            ...
            .order_by(EventoAntiTrampa.timestamp)
            .all()
        )
        ...
        analisis["nivel_riesgo"] = self._calcular_nivel_riesgo(
            analisis["puntuacion_riesgo"]
        )

        # Generar alertas y recomendaciones
        analisis["alertas_activas"] = self._generar_alertas(
            analisis["eventos_por_tipo"]
        )
    ...
    def detectar_sesiones_multiples(
        ...
        for intento in intentos:
            ...
            eventos = (
                ...
                .order_by(EventoAntiTrampa.timestamp.desc())
                .limit(10)
                .all()
            )
        ...
        resultado = {
            ...
            "nivel_sospecha": (
                "alto"
                if len(intentos) > 1 and len(ips_unicas) > 1
                else "medio" if len(intentos) > 1 else "bajo"
            ),
        }
        ...
        if resultado["sospechoso"] and intentos:
            self.registrar_evento(
                ...
                datos_evento={
                    ...
                    "user_agents": list(user_agents_unicos)[
                        :3
    ...
    def analizar_patron_respuestas(
        self, db: Session, intento_id: int, respuestas_recientes: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        velocidades = []
        for _i, respuesta in enumerate(respuestas_recientes):
        ...
        if velocidades:
            ...
            respuestas_muy_rapidas = [
                v for v in velocidades if v < 5
            ]  # Menos de 5 segundos
            if (
                len(respuestas_muy_rapidas) > len(velocidades) * 0.3
            ):  # Más del 30% muy rápidas
        ...
        for respuesta in respuestas_recientes:
            if respuesta.get("es_correcta", False):
                ...
                max_consecutivas = max(
                    max_consecutivas, respuestas_correctas_consecutivas
                )
            else:
                respuestas_correctas_consecutivas = 0
        ...
        if opciones_seleccionadas:
            ...
            if patron_detectado:
                analisis["patrones_detectados"].append(
                    f"patron_repetitivo_{patron_detectado}"
                )
    ...
    def generar_reporte_integridad(
        self, db: Session, intento_id: int
    ) -> dict[str, Any]:
        ...
        eventos = (
            db.query(EventoAntiTrampa)
            .filter(EventoAntiTrampa.intento_id == intento_id)
            .order_by(EventoAntiTrampa.timestamp)
            .all()
        )

        # Análisis de patrones completo
        analisis_actividad = self.analizar_patron_actividad(
            db, intento_id, ventana_tiempo=999999
        )  # TODO el tiempo

        # Análisis de sesiones múltiples
        sesiones_multiples = self.detectar_sesiones_multiples(
            db, intento.estudiante_id, intento.examen_id
        )

        # Calcular métricas de integridad
        total_eventos = len(eventos)
        eventos_criticos = len(
            [
                e
                for e in eventos
                if e.tipo_evento
        ...
        if puntuacion_integridad >= 80:
            ...
            recomendacion_accion = (
                "Revisar manualmente - Posibles irregularidades menores"
            )
        ...
        return {
            ...
            "metricas_generales": {
                "duracion_intento_minutos": (
                    (intento.fecha_finalizacion or datetime.now(UTC))
            ...
            "resumen_ejecutivo": {
                ...
                "aspectos_positivos": self._generar_aspectos_positivos(
                    analisis_actividad
                ),
    ...
    def configurar_monitoreo_tiempo_real(
        self, db: Session, examen_id: int, configuracion: dict[str, Any]
    ) -> bool:
            ...
            examen.configuracion_anti_trampa.update(
                {
                    ...
                    "umbral_eventos_criticos": configuracion.get(
                        "umbral_eventos_criticos", 3
                    ),
                    "tiempo_inactividad_max": configuracion.get(
                        "tiempo_inactividad_max", 300
                    ),
                    "permitir_salida_pantalla_completa": configuracion.get(
                        "permitir_salida_pantalla_completa", False
                    ),
                    "max_cambios_pestana": configuracion.get("max_cambios_pestana", 5),
                    "max_cambios_aplicacion": configuracion.get(
                        "max_cambios_aplicacion", 2
                    ),
                    "bloquear_clic_derecho": configuracion.get(
                        "bloquear_clic_derecho", True
                    ),
                    "bloquear_teclas_especiales": configuracion.get(
                        "bloquear_teclas_especiales", True
                    ),
    ...
    def _generar_descripcion_evento(
        self, tipo_evento: TipoEvento, datos_evento: dict[str, Any]
    ) -> str:
    ...
    def _verificar_alertas_tiempo_real(
        self, db: Session, evento: EventoAntiTrampa
    ) -> None:
        """Verificar si el evento requiere alertas inmediatas."""
        # Obtener configuración del examen
        intento = (
            db.query(IntentoExamen)
            .filter(IntentoExamen.intento_id == evento.intento_id)
            .first()
        )
        ...
        if config.get("alertas_inmediatas", False):
            if evento.tipo_evento in self.eventos_sospechosos:
                limite_critico = self.eventos_sospechosos[evento.tipo_evento][
                    "limite_critico"
                ]

                # Contar eventos del mismo tipo en los últimos 10 minutos
                timestamp_limite = datetime.now(UTC) - timedelta(minutes=10)
                eventos_similares = (
                    db.query(EventoAntiTrampa)
                    .filter(
                        and_(
    ...
    def _analizar_patron_temporal(
        self, eventos: list[EventoAntiTrampa]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "porcentaje_rafaga": (
                round((eventos_en_rafaga / len(intervalos)) * 100, 2)
                if intervalos
                else 0
            ),
        }

    def _analizar_cambios_contexto(
        self, eventos: list[EventoAntiTrampa]
    ) -> dict[str, Any]:
        ...
        return {
            "ips_unicas": len(ips_unicas),
            "user_agents_unicos": len(user_agents_unicos),
            "cambio_contexto_sospechoso": len(ips_unicas) > 1
            or len(user_agents_unicos) > 2,
        }
        ...
        for tipo_str, cantidad in eventos_por_tipo.items():
            try:
                ...
                if tipo_evento in self.eventos_sospechosos:
                    ...
                    if cantidad >= limite:
                        alertas.append(
                            f"CRÍTICO: {cantidad} eventos de tipo {tipo_str} (límite: {limite})"
                        )
                    elif cantidad >= limite * 0.7:
                        alertas.append(
                            f"ADVERTENCIA: {cantidad} eventos de tipo {tipo_str} aproximándose al límite"
                        )
            except ValueError:
                continue
    ...
    def _generar_recomendaciones(self, analisis: dict[str, Any]) -> list[str]:
        ...
        if analisis["nivel_riesgo"] == "critico":
            recomendaciones.append(
                "ACCIÓN INMEDIATA: Considerar suspender el intento y contactar al estudiante"
            )
        ...
        if eventos_cambio_app > 2:
            recomendaciones.append(
                f"Investigar {eventos_cambio_app} cambios de aplicación"
            )

        eventos_pantalla_completa = analisis["eventos_por_tipo"].get(
            "pantalla_completa_salida", 0
        )
    ...
    def _detectar_patron_repetitivo(self, secuencia: list[Any]) -> str | None:
    ...
    def _generar_flags_rojas(
        self, eventos: list[EventoAntiTrampa], analisis: dict[str, Any]
    ) -> list[str]:
        ...
        if analisis["puntuacion_riesgo"] > 50:
            flags.append(
                f"Puntuación de riesgo muy alta: {analisis['puntuacion_riesgo']}"
            )
    ...
    def _generar_recomendaciones_detalladas(
        ...
        if puntuacion_integridad >= 80:
            recomendaciones.append(
                "✅ Intento con alta integridad - Se puede calificar normalmente"
            )

        elif puntuacion_integridad >= 60:
            recomendaciones.append("⚠️ Revisar respuestas manualmente por precaución")
            recomendaciones.append(
                "💬 Considerar conversación con el estudiante sobre las irregularidades menores"
            )

        elif puntuacion_integridad >= 40:
            recomendaciones.append(
                "🔍 Investigación requerida - Múltiples eventos sospechosos"
            )
            recomendaciones.append(
                "📞 Entrevista con el estudiante altamente recomendada"
            )
            ...
            recomendaciones.append(
                "📋 Ofrecer al estudiante la oportunidad de repetir bajo supervisión"
            )

        # Recomendaciones específicas por tipo de evento
        if sesiones_multiples.get("sospechoso", False):
            recomendaciones.append(
                "🔐 Verificar que el estudiante no compartió credenciales"
            )

        cambios_app = analisis_actividad["eventos_por_tipo"].get("cambio_aplicacion", 0)
        if cambios_app > 0:
            recomendaciones.append(
                f"💻 Investigar los {cambios_app} cambios de aplicación detectados"
            )
