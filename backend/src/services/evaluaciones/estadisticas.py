...
class ServicioEstadisticas:
    ...
    def generar_estadisticas_examen(
        self, db: Session, examen_id: int, incluir_intentos_incompletos: bool = False
    ) -> dict[str, Any]:
        ...
        preguntas = (
            db.query(PreguntaExamen).filter(PreguntaExamen.examen_id == examen_id).all()
        )
        ...
        estadisticas = {
            ...
            "metricas_generales": self._calcular_metricas_generales(
                intentos, examen.puntuacion_total
            ),
            "distribucion_calificaciones": self._calcular_distribucion_calificaciones(
                intentos, examen.puntuacion_total
            ),
            "analisis_temporal": self._analizar_tiempos_resolucion(intentos),
            "analisis_preguntas": self._analizar_preguntas_detalle(
                db, preguntas, examen_id
            ),
            "rendimiento_por_estudiante": self._analizar_rendimiento_estudiantes(
                db, intentos
            ),
            "comparacion_grupos": self._comparar_grupos_rendimiento(db, intentos),
            # Metadatos del análisis
            "metadata": {
                "fecha_analisis": datetime.now(UTC).isoformat(),
    ...
    def generar_reporte_clase(
        ...
        if not periodo_inicio:
            periodo_inicio = datetime.now(UTC) - timedelta(days=90)  # Últimos 3 meses
        if not periodo_fin:
            periodo_fin = datetime.now(UTC)
        ...
        for examen in examenes:
            stats_examen = self.generar_estadisticas_examen(
                ...
                analisis_examenes.append(
                    {
                        ...
                        "promedio_clase": stats_examen["metricas_generales"][
                            "promedio_calificaciones"
                        ],
                        "tasa_aprobacion": stats_examen["metricas_generales"][
                            "tasa_aprobacion"
                        ],
                        "total_intentos": stats_examen["metadata"][
                            "total_intentos_analizados"
                        ],
                    }
                )
        ...
        return {
            ...
            "resumen_ejecutivo": self._generar_resumen_ejecutivo_clase(
                examenes, intentos_clase
            ),
            "tendencias_rendimiento": self._analizar_tendencias_clase(
                analisis_examenes
            ),
            "estudiantes_destacados": self._identificar_estudiantes_destacados(
                db, intentos_clase
            ),
            "estudiantes_en_riesgo": self._identificar_estudiantes_riesgo(
                db, intentos_clase
            ),
            "analisis_por_examen": analisis_examenes,
            "recomendaciones": self._generar_recomendaciones_clase(
                intentos_clase, analisis_examenes
            ),
            "metadata": {
    ...
    def analizar_pregunta_detallada(
        self, db: Session, pregunta_id: int
    ) -> dict[str, Any]:
        """Análisis detallado de una pregunta específica."""
        pregunta = (
            db.query(PreguntaExamen)
            .filter(PreguntaExamen.pregunta_id == pregunta_id)
            .first()
        )
        ...
        return {
            ...
            "estadisticas_respuestas": self._analizar_respuestas_pregunta(
                respuestas, pregunta
            ),
            "analisis_dificultad": self._evaluar_dificultad_real(respuestas, pregunta),
            "patrones_respuestas_incorrectas": self._analizar_errores_comunes(
                respuestas, pregunta
            ),
            "discriminacion": self._calcular_discriminacion_pregunta(
                db, pregunta, respuestas
            ),
            "recomendaciones_mejora": self._generar_recomendaciones_pregunta(
                respuestas, pregunta
            ),
        }

    def generar_dashboard_profesor(
        self, db: Session, profesor_id: int, periodo_dias: int = 30
    ) -> dict[str, Any]:
        """Generar dashboard con métricas para el profesor."""
        fecha_inicio = datetime.now(UTC) - timedelta(days=periodo_dias)
        ...
        for examen in examenes_profesor:
            ...
            calificaciones_todas.extend(
                [
                    i.puntuacion_obtenida
                    for i in intentos
                    if i.puntuacion_obtenida is not None
                ]
            )
        ...
        return {
            "resumen_periodo": {
                ...
                "promedio_general": (
                    round(statistics.mean(calificaciones_todas), 2)
            ...
            "examenes_recientes": [
                {
                    ...
                    "intentos_completados": db.query(IntentoExamen)
                    .filter(
                        and_(
                ...
                for e in sorted(
                    examenes_profesor, key=lambda x: x.fecha_creacion, reverse=True
                )[:5]
            ],
            "alertas_atencion": self._generar_alertas_profesor(db, examenes_profesor),
            "metricas_rapidas": self._calcular_metricas_rapidas_profesor(
                db, examenes_profesor
            ),
        }

    def exportar_estadisticas(
        self, db: Session, examen_id: int, formato: str = "json"
    ) -> dict[str, Any]:
    ...
    def _calcular_metricas_generales(
        self, intentos: list[IntentoExamen], puntuacion_maxima: float
    ) -> dict[str, Any]:
        """Calcular métricas estadísticas básicas."""
        calificaciones = [
            i.puntuacion_obtenida for i in intentos if i.puntuacion_obtenida is not None
        ]
        tiempos = [
        ...
        return {
            ...
            "desviacion_estandar": round(
                statistics.stdev(calificaciones) if len(calificaciones) > 1 else 0, 2
            ),
            ...
            "tiempo_promedio_minutos": (
                round(statistics.mean(tiempos), 2) if tiempos else 0
            ),
            "tiempo_mediano_minutos": (
                round(statistics.median(tiempos), 2) if tiempos else 0
            ),
        }

    def _calcular_distribucion_calificaciones(
        self, intentos: list[IntentoExamen], puntuacion_maxima: float
    ) -> dict[str, Any]:
        ...
        rangos = {"0-20%": 0, "21-40%": 0, "41-60%": 0, "61-80%": 0, "81-100%": 0}
    ...
    def _analizar_tiempos_resolucion(
        self, intentos: list[IntentoExamen]
    ) -> dict[str, Any]:
        ...
        for intento in intentos:
            if intento.fecha_finalizacion and intento.fecha_inicio:
                tiempo = (
                    intento.fecha_finalizacion - intento.fecha_inicio
                ).seconds // 60
        ...
        return {
            ...
            "desviacion_tiempo": round(
                statistics.stdev(tiempos_minutos) if len(tiempos_minutos) > 1 else 0, 2
            ),
            "abandonos": len(patrones_abandono),
            "tasa_abandono": (
                round((len(patrones_abandono) / len(intentos)) * 100, 2)
                if intentos
                else 0
            ),
        }

    def _analizar_preguntas_detalle(
        self, db: Session, preguntas: list[PreguntaExamen], examen_id: int
    ) -> list[dict[str, Any]]:
        ...
        for pregunta in preguntas:
            respuestas = (
                db.query(RespuestaEstudiante)
                .filter(RespuestaEstudiante.pregunta_id == pregunta.pregunta_id)
                .all()
            )
            ...
            analisis_pregunta = {
                ...
                "estadisticas": {
                    ...
                    "tasa_acierto": round(
                        (respuestas_correctas / total_respuestas) * 100, 2
                    ),
                    "promedio_puntuacion": (
                        round(statistics.mean(puntuaciones), 2) if puntuaciones else 0
                    ),
                    "dificultad_real": self._calcular_dificultad_real(
                        respuestas_correctas, total_respuestas
                    ),
                },
                "tiempo_promedio": (
                    round(
                        statistics.mean(
                            [
                                r.tiempo_respuesta
                                for r in respuestas
                                if r.tiempo_respuesta
                            ]
                        ),
                        2,
                    )
                    if any(r.tiempo_respuesta for r in respuestas)
                    else 0
                ),
            }
            ...
            if pregunta.tipo_pregunta == TipoPregunta.OPCION_MULTIPLE:
                analisis_pregunta["distribucion_opciones"] = (
                    self._analizar_opciones_multiple(respuestas)
                )
    ...
    def _analizar_rendimiento_estudiantes(
        self, db: Session, intentos: list[IntentoExamen]
    ) -> list[dict[str, Any]]:
        ...
        for intento in intentos:
            ...
            if estudiante_id not in estudiantes_rendimiento:
                # Obtener información del estudiante
                usuario = db.query(Usuario).filter(Usuario.usuario_id == estudiante_id).first()
                estudiantes_rendimiento[estudiante_id] = {
            ...
            if intento.puntuacion_obtenida is not None:
                estudiantes_rendimiento[estudiante_id]["intentos"].append(
                    {
                        "intento_id": intento.intento_id,
                        "puntuacion": intento.puntuacion_obtenida,
                        "fecha": intento.fecha_inicio.isoformat(),
                        "tiempo_minutos": (
                            (intento.fecha_finalizacion - intento.fecha_inicio).seconds
                            // 60
        ...
        for estudiante_id, datos in estudiantes_rendimiento.items():
    ...
    def _comparar_grupos_rendimiento(
        self, db: Session, intentos: list[IntentoExamen]
    ) -> dict[str, Any]:
    ...
    def _analizar_respuestas_pregunta(
        self, respuestas: list[RespuestaEstudiante], pregunta: PreguntaExamen
    ) -> dict[str, Any]:
    ...
    def _evaluar_dificultad_real(
        ...
        return {
            ...
            "recomendacion": self._recomendar_ajuste_dificultad(
                pregunta.dificultad.value, dificultad_real
            ),
        }

    def _analizar_errores_comunes(
        self, respuestas: list[RespuestaEstudiante], pregunta: PreguntaExamen
    ) -> dict[str, Any]:
        ...
        if pregunta.tipo_pregunta == TipoPregunta.OPCION_MULTIPLE:
            ...
            for respuesta in respuestas_incorrectas:
                if (
                    ...
                    and "opcion_seleccionada" in respuesta.respuesta_estudiante
                ):
                    opcion = respuesta.respuesta_estudiante["opcion_seleccionada"]
                    opciones_incorrectas[opcion] = (
                        opciones_incorrectas.get(opcion, 0) + 1
                    )

            return {
                "total_incorrectas": len(respuestas_incorrectas),
                "opciones_incorrectas_frecuentes": dict(
                    sorted(
                        opciones_incorrectas.items(), key=lambda x: x[1], reverse=True
                    )
                ),
                "patron_principal": (
                    max(opciones_incorrectas, key=opciones_incorrectas.get)
                    ...
                    for palabra in palabras:
                        if len(palabra) > 3:  # Ignorar palabras muy cortas
                            palabras_frecuentes[palabra] = (
                                palabras_frecuentes.get(palabra, 0) + 1
                            )

            return {
                "total_incorrectas": len(respuestas_incorrectas),
                "palabras_frecuentes_incorrectas": dict(
                    sorted(
                        palabras_frecuentes.items(), key=lambda x: x[1], reverse=True
                    )[:10]
                ),  # Top 10
            }
    ...
    def _calcular_discriminacion_pregunta(
        ...
        for respuesta in respuestas:
            intento = (
                db.query(IntentoExamen)
                .filter(IntentoExamen.intento_id == respuesta.intento_id)
                .first()
            )
            if intento and intento.puntuacion_obtenida is not None:
                intentos_puntuaciones[respuesta.intento_id] = (
        ...
        puntuaciones_ordenadas = sorted(
            intentos_puntuaciones.items(), key=lambda x: x[1], reverse=True
        )
        n_grupo = max(1, len(puntuaciones_ordenadas) // 4)  # 25% aproximadamente

        grupo_alto_ids = {item[0] for item in puntuaciones_ordenadas[:n_grupo]}
        grupo_bajo_ids = {item[0] for item in puntuaciones_ordenadas[-n_grupo:]}

        # Calcular aciertos en cada grupo
        aciertos_alto = len(
            [r for r in respuestas if r.intento_id in grupo_alto_ids and r.es_correcta]
        )
        aciertos_bajo = len(
            [r for r in respuestas if r.intento_id in grupo_bajo_ids and r.es_correcta]
        )
    ...
    def _generar_recomendaciones_pregunta(
        ...
        if tasa_acierto < 0.2:
            recomendaciones.append(
                "🔴 Pregunta muy difícil - considerar revisar la redacción o reducir la dificultad"
            )
            recomendaciones.append(
                "📝 Verificar que la respuesta correcta esté claramente definida"
            )

        elif tasa_acierto > 0.9:
            recomendaciones.append(
                "🟢 Pregunta muy fácil - considerar aumentar la complejidad"
            )
            recomendaciones.append(
                "🎯 Agregar distractores más plausibles en opciones múltiples"
            )
        ...
        if pregunta.tipo_pregunta == TipoPregunta.OPCION_MULTIPLE:
            if tasa_acierto > 0.8:
                recomendaciones.append(
                    "⚡ Mejorar los distractores para aumentar el desafío"
                )
            ...
            and tasa_acierto < 0.4
        ):
            recomendaciones.append(
                "📋 Considerar proporcionar más criterios de evaluación claros"
            )
        ...
        if tiempos:
            ...
            if tiempo_promedio < 10:  # Menos de 10 segundos
                recomendaciones.append(
                    "⏱️ Respuestas muy rápidas - podría ser demasiado fácil"
                )
            elif tiempo_promedio > 300:  # Más de 5 minutos
                recomendaciones.append(
                    "⏳ Respuestas muy lentas - considerar clarificar la pregunta"
                )

        if not recomendaciones:
            recomendaciones.append(
                "✅ Pregunta funcionando correctamente - sin ajustes necesarios"
            )

        return recomendaciones

    def _guardar_estadisticas_examen(
        self, db: Session, examen_id: int, estadisticas: dict[str, Any]
    ) -> None:
        """Guardar estadísticas en la base de datos."""
        try:
            # Buscar registro existente
            estadistica_existente = (
                db.query(EstadisticaExamen)
                .filter(EstadisticaExamen.examen_id == examen_id)
                .first()
            )
            ...
            if estadistica_existente:
                ...
                estadistica_existente.fecha_actualizacion = datetime.now(UTC)
            else:
    ...
    def _analizar_opciones_multiple(
        self, respuestas: list[RespuestaEstudiante]
    ) -> dict[str, Any]:
        ...
        for opcion, datos in distribucion.items():
            datos["porcentaje_seleccion"] = round(
                (datos["total"] / total_respuestas) * 100, 2
            )
            datos["es_correcta"] = datos["correctas"] > 0

        return distribucion

    def _recomendar_ajuste_dificultad(
        self, dificultad_config: str, dificultad_real: str
    ) -> str:
        """Recomendar ajustes basados en diferencia entre dificultad configurada y real."""
        config_orden = self.dificultades_orden.get(
            DificultadPregunta(dificultad_config), 3
        )
    ...
    def _generar_resumen_ejecutivo_clase(
        self, examenes: list[Examen], intentos: list[IntentoExamen]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "total_intentos": len(intentos),
            "promedio_clase": (
                round(statistics.mean(calificaciones), 2) if calificaciones else 0
            ),
            "estudiantes_unicos": len({i.estudiante_id for i in intentos}),
            "tasa_participacion": (
                round((len(intentos) / len(examenes)) * 100, 2) if examenes else 0
            ),
            "ultimo_examen": (
                max([e.fecha_creacion for e in examenes]) if examenes else None
            ),
        }

    def _analizar_tendencias_clase(
        self, analisis_examenes: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """Analizar tendencias de rendimiento en la clase."""
        if len(analisis_examenes) < 2:
            return {
                "mensaje": "Necesarios al menos 2 exámenes para analizar tendencias"
            }
        ...
        return {
            "tendencia_promedio": {
                "direccion": (
                    ...
                    else "empeorando" if mejora_promedio < 0 else "estable"
                ),
            ...
            "tendencia_aprobacion": {
                "direccion": (
                    "mejorando"
                    if mejora_aprobacion > 0
                    else "empeorando" if mejora_aprobacion < 0 else "estable"
                ),
                "cambio": round(mejora_aprobacion, 2),
            },
            "consistencia": (
                round(statistics.stdev(promedios), 2) if len(promedios) > 1 else 0
            ),
        }

    def _identificar_estudiantes_destacados(
        self, db: Session, intentos: list[IntentoExamen]
    ) -> list[dict[str, Any]]:
        ...
        for intento in intentos:
            if intento.puntuacion_obtenida is not None:
                estudiante_id = intento.estudiante_id
                if estudiante_id not in rendimiento_estudiantes:
                    rendimiento_estudiantes[estudiante_id] = []
                rendimiento_estudiantes[estudiante_id].append(
                    intento.puntuacion_obtenida
                )

        destacados = []
        for estudiante_id, puntuaciones in rendimiento_estudiantes.items():
            if len(puntuaciones) >= 2:  # Al menos 2 exámenes
                promedio = statistics.mean(puntuaciones)
                if promedio >= 85:  # 85% o más
                    usuario = (
                        db.query(Usuario).filter(Usuario.usuario_id == estudiante_id).first()
                    )
                    destacados.append(
                        {
                            "estudiante_id": estudiante_id,
        ...
        return sorted(destacados, key=lambda x: x["promedio"], reverse=True)[
            :5
        ]  # Top 5

    def _identificar_estudiantes_riesgo(
        self, db: Session, intentos: list[IntentoExamen]
    ) -> list[dict[str, Any]]:
        ...
        return sorted(
            en_riesgo, key=lambda x: x["promedio"]
        )  # Ordenar por promedio ascendente

    def _generar_recomendaciones_clase(
        self, intentos: list[IntentoExamen], analisis_examenes: list[dict[str, Any]]
    ) -> list[str]:
        ...
        if calificaciones:
            ...
            if promedio_general < 60:
                recomendaciones.append(
                    "🚨 Promedio de clase bajo - considerar revisar metodología de enseñanza"
                )
                recomendaciones.append("📚 Proporcionar material de estudio adicional")

            elif promedio_general > 85:
                recomendaciones.append(
                    "🎯 Clase con alto rendimiento - considerar aumentar el nivel de desafío"
                )

        # Análisis de participación
        if len(analisis_examenes) > 0:
            participacion_promedio = statistics.mean(
                [e["total_intentos"] for e in analisis_examenes]
            )
            if participacion_promedio < 50:  # Menos del 50% de participación esperada
                recomendaciones.append(
                    "📢 Baja participación - considerar estrategias para motivar a los estudiantes"
                )
    ...
    def _generar_alertas_profesor(
        self, db: Session, examenes: list[Examen]
    ) -> list[dict[str, Any]]:
        ...
        for examen in examenes:
            ...
            if (
                examen.fecha_limite
                and examen.fecha_limite <= datetime.now(UTC) + timedelta(days=2)
            ):
                alertas.append(
                    {
        ...
        return sorted(
            alertas,
            key=lambda x: {"alta": 3, "media": 2, "baja": 1}[x["prioridad"]],
            reverse=True,
        )

    def _calcular_metricas_rapidas_profesor(
        self, db: Session, examenes: list[Examen]
    ) -> dict[str, Any]:
        ...
        for examen in examenes:
            ...
            intentos = (
                db.query(IntentoExamen)
                .filter(IntentoExamen.examen_id == examen.examen_id)
                .all()
            )

            total_intentos += len(intentos)
            total_estudiantes.update(i.estudiante_id for i in intentos)

        return {
            "examenes_activos": examenes_activos,
            "total_intentos_periodo": total_intentos,
            "estudiantes_activos": len(total_estudiantes),
            "promedio_intentos_por_examen": (
                round(total_intentos / len(examenes), 1) if examenes else 0
            ),
        }
