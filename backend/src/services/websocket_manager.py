...
from api.dependencies import get_current_user_websocket
from models.users import Usuario
from models.communication.chat import SalaChat, Mensaje, TipoMensaje, EstadoMensaje
from crud.communication.chat import (
    crud_sala_chat, crud_participante_sala, crud_mensaje, 
    crud_lectura_mensaje, crud_notificacion
)
from schemas.communication.chat_schemas import MensajeCreate, NotificacionCreate
...
class ConnectionManager:
    ...
    async def connect(
        ...
        if connection_id is None:
            connection_id = f"{usuario_id}_{datetime.now().timestamp()}"
    ...
    async def disconnect(self, connection_id: str):
        ...
        if usuario_id in self.active_connections:
            ...
            if not self.active_connections[usuario_id]:
                ...
                if usuario_id in self.user_activity:
                    ...
                    self.user_activity[usuario_id]["last_activity"] = datetime.now()
                
                # Remover de salas
                for sala, participants in self.sala_participants.items():
        ...
        logger.info(f"Usuario {usuario_id} desconectado. Connection ID: {connection_id}")
    
    async def join_sala(self, usuario_id: str, sala_id: str):
    ...
    async def leave_sala(self, usuario_id: str, sala_id: str):
    ...
    async def send_personal_message(self, usuario_id: str, message: Dict[str, Any]):
        ...
        if usuario_id in self.active_connections:
            ...
            for connection_id, websocket in self.active_connections[usuario_id].items():
                try:
                    ...
                    logger.error(f"Error enviando mensaje a {usuario_id}: {e}")
...
class WebSocketHandler:
    ...
    async def _handle_join_sala(self, data: Dict[str, Any]):
    ...
    async def _handle_leave_sala(self, data: Dict[str, Any]):
    ...
    async def _handle_get_online_users(self, websocket: WebSocket, data: Dict[str, Any]):
        """Obtener usuarios online en una sala"""
        sala_id = data["sala_id"]
    ...
    async def _handle_rutilio_mention(self, mensaje: Mensaje):
        ...
        try:
            ...
            logger.error(f"Error procesando mención de Rutilio: {e}")
    ...
    async def _generate_rutilio_response(self, contenido: str) -> str:
        ...
        elif "ayuda" in contenido_lower or "help" in contenido_lower:
        ...
        elif "examen" in contenido_lower or "test" in contenido_lower:
        ...
        elif "gracias" in contenido_lower or "thanks" in contenido_lower:
    ...
    async def _create_mention_notifications(self, mensaje: Mensaje):
        ...
        try:
            ...
            if mensaje.menciones_todos:
                ...
                for participante in participantes:
                    if participante.usuario_id != self.usuario.id:  # No notificar al remitente
                        notificacion = NotificacionCreate(
            ...
            logger.error(f"Error creando notificaciones de mención: {e}")
...
async def websocket_endpoint(
    ...
    try:
        ...
        # Conectar usuario
        connection_id = f"{usuario.id}_{datetime.now().timestamp()}"
