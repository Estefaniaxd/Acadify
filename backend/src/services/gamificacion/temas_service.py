...
class TemasService:
    ...
    def comprar_tema(db: Session, usuario_id: UUID, tema_id: UUID, costo: int = 10):
        ...
        if not usuario_puntos or usuario_puntos.puntos_acumulados < costo:
            msg = "No tienes suficientes puntos para comprar este tema"
            raise ValueError(msg)
        # Verificar que el usuario no tenga ya el tema
        ya_tiene = (
            db.query(TemaPersonalizado)
            .filter_by(usuario_id=usuario_id, tema_id=tema_id)
            .first()
        )
        if ya_tiene:
            msg = "Ya tienes este tema"
            raise ValueError(msg)
    ...
    def listar_temas_disponibles(db: Session, usuario_id: UUID):
        ...
        return db.query(Tema).filter(~Tema.tema_id.in_(subq)).all()
