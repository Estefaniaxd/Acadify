"""
Servicio para lógica de puntos y rachas de usuario.
"""
...
class PuntosService:
    @staticmethod
    def asignar_puntos(db: Session, usuario_id: uuid.UUID, puntos: int, motivo: str = None):
        """
    ...
    @staticmethod
    def descontar_puntos(db: Session, usuario_id: uuid.UUID, puntos: int, motivo: str = None):
        """
    ...
    @staticmethod
    def actualizar_racha(db: Session, usuario_id: uuid.UUID):
        """
        ...
        if not racha:
            racha = RachaUsuario(usuario_id=usuario_id, racha_actual=1, mejor_racha=1, fecha_ultimo_dia=hoy)
            db.add(racha)
        else:
    ...
    @staticmethod
    def obtener_racha(db: Session, usuario_id: uuid.UUID) -> int:
        racha = db.query(RachaUsuario).filter_by(usuario_id=usuario_id).first()
