...
import src.crud.classes.tarea as crud_tarea
from src.schemas.classes.tarea import Tarea, TareaCreate, TareaUpdate
...
@router.get("/clase/{clase_id}", response_model=list[Tarea])
...
@router.get("/docente/{docente_id}", response_model=list[Tarea])
...
@router.get("/activas", response_model=list[Tarea])
def get_tareas_activas(db: Session = Depends(get_db)):
    """Obtener tareas activas."""
    return crud_tarea.get_active_tasks(db)


@router.get("/vencidas", response_model=list[Tarea])
...
@router.get("/rango-fechas", response_model=list[Tarea])
...
@router.get("/con-entregas-tardias", response_model=list[Tarea])
