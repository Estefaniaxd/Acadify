...
from src.schemas.classes.clase import Clase as ClaseSchema
...
def read_clases(
    ...
    return clase.get_multi(db=db, skip=skip, limit=limit)
...
def read_clases_by_grupo_curso(
    ...
    return clase.get_by_grupo_curso(db=db, grupo_curso_id=grupo_curso_id)
...
def read_clases_by_plataforma(
    ...
    return clase.get_by_plataforma(db=db, plataforma_id=plataforma_id)


@router.get("/proximas/", response_model=list[ClaseSchema])
def read_upcoming_clases(
    limit: int = Query(
        10, ge=1, le=50, description="Número máximo de clases próximas a retornar"
    ),
    db: Session = Depends(get_db),
) -> list[ClaseSchema]:
    """Obtener las próximas clases ordenadas por fecha de inicio."""
    return clase.get_upcoming_classes(db=db, limit=limit)
...
def read_clases_by_date_range(
    fecha_inicio: datetime = Query(
        ...,
        description="Fecha y hora de inicio del rango (formato: 2024-01-01T10:00:00)",
    ),
    fecha_fin: datetime = Query(
        ..., description="Fecha y hora de fin del rango (formato: 2024-01-31T18:00:00)"
    ),
    db: Session = Depends(get_db),
) -> list[ClaseSchema]:
    ...
    return clase.get_by_date_range(db=db, start_date=fecha_inicio, end_date=fecha_fin)
...
def search_clases(
    grupo_curso_id: UUID | None = Query(None, description="Filtrar por grupo curso"),
    plataforma_id: UUID | None = Query(None, description="Filtrar por plataforma"),
    fecha_desde: date | None = Query(
        None, description="Filtrar desde esta fecha (formato: YYYY-MM-DD)"
    ),
    fecha_hasta: date | None = Query(
        None, description="Filtrar hasta esta fecha (formato: YYYY-MM-DD)"
    ),
    skip: int = Query(0, ge=0, description="Número de registros a omitir"),
    limit: int = Query(
        100, ge=1, le=1000, description="Máximo número de registros a retornar"
    ...
    return query.order_by(Clase.hora_inicio).offset(skip).limit(limit).all()
...
def read_clases_today(
    ...
    return clase.get_by_date_range(db=db, start_date=start_of_day, end_date=end_of_day)
...
def read_clases_this_week(
    ...
    return clase.get_by_date_range(
        db=db, start_date=start_datetime, end_date=end_datetime
    )
...
def update_clase_hora(
    clase_id: UUID,
    nueva_hora: datetime = Query(
        ..., description="Nueva hora de inicio (formato: 2024-01-01T10:00:00)"
    ),
    db: Session = Depends(get_db),
) -> ClaseSchema:
    ...
    if nueva_hora <= datetime.now(UTC):
        raise HTTPException(
            status_code=400, detail="La nueva hora debe ser en el futuro"
        )
