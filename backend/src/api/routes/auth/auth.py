...
async def login(
    ...
    if not usuario or not password_service.verify_password(
    ...
    refresh_token, _ = token_service.create_refresh_token(
        user_id=str(usuario.usuario_id), expires_delta=refresh_token_expires
    )
...
@router.post(
    "/register",
    response_model=usuario_schemas.UsuarioRead,
    summary="Registrar un nuevo usuario",
    description="Crea un nuevo usuario en el sistema.",
)
def register_user(
    usuario_in: usuario_schemas.UsuarioCreate, db: Session = Depends(get_db)
):
    usuario_existente = crud_usuario.get_by_email(
        db, email=usuario_in.correo_institucional
    )
...
async def refresh_token(
    ...
    new_refresh_token, _ = token_service.create_refresh_token(
        user_id=str(usuario.usuario_id), expires_delta=timedelta(days=7)
    )
    response.set_cookie(
...
async def google_callback(
    ...
    try:
        ...
        user_info = await oauth_service.get_user_info_from_google(
...
async def forgot_password(email: str, db: Session = Depends(get_db)):
    ...
    if not usuario:
        return {
            "detail": "Si el correo existe, se ha enviado un enlace para restablecer la contraseña."
        }
    ...
    email_service.send_password_reset_email(
        email_to=usuario.email, username=usuario.nombre, reset_link=reset_link
    )
...
async def reset_password(
    reset_data: auth_schemas.PasswordResetRequest, db: Session = Depends(get_db)
):
