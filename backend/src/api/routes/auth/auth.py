...
async def verify_email(
    ...
    **Respuestas:**
    ...
    try:
        ...
        logger.exception(f"Error en verificación de email: {e}")
        raise HTTPException(
            status_code=400, detail="No se pudo verificar el correo"
        ) from e
...
async def setup_2fa(
    *,
    ...
    try:
        return await auth_service.setup_2fa(db, current_user.usuario_id, setup_request)
    except Exception as e:
        logger.exception(f"Error configurando 2FA: {e}")
        raise
...
async def verify_2fa_setup(
    *,
    ...
    try:
        result = await auth_service.verify_2fa_setup(
            db, current_user.usuario_id, verify_request
        )
        ...
        logger.exception(f"Error verificando 2FA setup: {e}")
        raise
...
async def disable_2fa(
    *,
    ...
    try:
        ...
        logger.exception(f"Error desactivando 2FA: {e}")
        raise
...
async def get_2fa_status(
    ...
    return TwoFAStatusResponse(
        twofa_enabled=current_user.twofa_enabled, twofa_method=twofa_method
    )
...
@router.post(
    "/users", response_model=UserCurrentResponse, status_code=status.HTTP_201_CREATED
)
...
async def activate_user_account(
    *,
    ...
    try:
        ...
        logger.exception(f"Error activando cuenta {user_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ERROR_INTERNO_SERVIDOR,
        ) from e
...
async def deactivate_user_account(
    *,
    ...
    try:
        ...
        logger.exception(f"Error desactivando cuenta {user_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ERROR_INTERNO_SERVIDOR,
        ) from e
...
async def unlock_user_account(
    *,
    ...
    try:
        ...
        logger.exception(f"Error desbloqueando cuenta {user_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ERROR_INTERNO_SERVIDOR,
        ) from e
...
async def auth_health_check(
    ...
    return {
        ...
        "components": {"redis": redis_status, "auth_service": "healthy"},
    }
