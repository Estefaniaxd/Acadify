...
async def request_email_verification(
    *,
    ...
    auth_service = AuthService(redis_client)
    try:
        return await auth_service.request_email_verification(
...
async def request_account_deletion(
    *,
    ...
    auth_service = AuthService(redis_client)
    try:
        return await auth_service.request_account_deletion(
            db, current_user.usuario_id, data.password
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def confirm_account_deletion(
    *,
    ...
    auth_service = AuthService(redis_client)
    try:
        result = await auth_service.confirm_account_deletion(
            db, current_user.usuario_id, data.deletion_code
        )
