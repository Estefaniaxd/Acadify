...
@router.post(
    "/mensaje-bots/", response_model=MensajeBot, status_code=status.HTTP_201_CREATED
)
def create_mensaje_bot(
    *, db: Session = Depends(get_db), mensaje_bot_in: MensajeBotCreate
) -> Any:
    """Crear nuevo mensaje de bot."""
    return crud_mensaje_bot.create(db=db, obj_in=mensaje_bot_in)
...
def read_mensaje_bots(
    ...
    return crud_mensaje_bot.get_multi(db, skip=skip, limit=limit)


@router.get("/mensaje-bots/{mensaje_bot_id}", response_model=MensajeBot)
def read_mensaje_bot(*, db: Session = Depends(get_db), mensaje_bot_id: UUID) -> Any:
    """Obtener mensaje bot por ID."""
    mensaje_bot = crud_mensaje_bot.get(db=db, mensaje_bot_id=mensaje_bot_id)
    if not mensaje_bot:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=MENSAJE_BOT_NOT_FOUND
        )
    return mensaje_bot
...
def read_mensaje_bots_by_usuario(
    *, db: Session = Depends(get_db), usuario_id: UUID
) -> Any:
...
def read_mensaje_bots_by_material(
    *, db: Session = Depends(get_db), material_id: UUID
) -> Any:
...
def read_mensaje_bots_by_contexto(
    *, db: Session = Depends(get_db), contexto: ContextoMensaje
) -> Any:
...
def search_mensaje_bots(*, db: Session = Depends(get_db), search_term: str) -> Any:
...
def read_conversation_history(
    *,
    ...
    return crud_mensaje_bot.get_conversation_history(
        db, usuario_id=usuario_id, chat_grupo_id=chat_grupo_id, limit=limit
    )


@router.get("/mensaje-bots/sin-respuesta", response_model=list[MensajeBot])
def read_unanswered_mensaje_bots(db: Session = Depends(get_db)) -> Any:
...
def read_mensaje_bot_count_by_contexto(db: Session = Depends(get_db)) -> Any:
    """Contar mensajes bot por contexto."""
    return mensaje_bot.count_messages_by_context(db)


@router.get(
    "/mensaje-bots/estadisticas/usuarios-activos", response_model=dict[str, int]
)
...
def update_mensaje_bot(
    *,
    ...
    return crud_mensaje_bot.update(db=db, db_obj=mensaje_bot, obj_in=mensaje_bot_in)
...
def update_mensaje_bot_response(
    *,
    ...
    mensaje_bot = crud_mensaje_bot.update_response(
        db=db, mensaje_bot_id=mensaje_bot_id, new_response=new_response
    )
    if not mensaje_bot:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=MENSAJE_BOT_NOT_FOUND
...
def delete_mensaje_bot(*, db: Session = Depends(get_db), mensaje_bot_id: UUID) -> Any:
