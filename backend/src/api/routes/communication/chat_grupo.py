...
@router.post(
    "/chat-grupos/", response_model=ChatGrupo, status_code=status.HTTP_201_CREATED
)
def create_chat_grupo(
    *, db: Session = Depends(get_db), chat_grupo_in: ChatGrupoCreate
) -> Any:
    """
...
def read_chat_grupo(*, db: Session = Depends(get_db), chat_grupo_id: UUID) -> Any:
    """
    ...
    if not chat_grupo:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Chat de grupo no encontrado"
        )
    return chat_grupo


@router.get("/chat-grupos/grupo/{grupo_id}", response_model=ChatGrupo)
def read_chat_grupo_by_grupo(*, db: Session = Depends(get_db), grupo_id: UUID) -> Any:
    """
...
def read_chat_grupos_by_estado(
    *, db: Session = Depends(get_db), estado: EstadoChatGrupo
) -> Any:
    """
...
def read_active_chat_grupos(db: Session = Depends(get_db)) -> Any:
    """
...
def read_archived_chat_grupos(db: Session = Depends(get_db)) -> Any:
    """
...
def read_chat_grupos_with_files(db: Session = Depends(get_db)) -> Any:
    """
...
def archive_chat_grupo(*, db: Session = Depends(get_db), chat_grupo_id: UUID) -> Any:
    """
...
def activate_chat_grupo(*, db: Session = Depends(get_db), chat_grupo_id: UUID) -> Any:
    """
...
def update_chat_storage(
    *,
    ...
    chat_grupo = crud_chat_grupo.update_storage_capacity(
        db=db, chat_grupo_id=chat_grupo_id, new_capacity=new_capacity
    )
    if not chat_grupo:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Chat de grupo no encontrado"
...
def toggle_file_permissions(
    *, db: Session = Depends(get_db), chat_grupo_id: UUID
    ...
    """
    chat_grupo = crud_chat_grupo.toggle_file_permissions(
...
def delete_chat_grupo(*, db: Session = Depends(get_db), chat_grupo_id: UUID) -> Any:
    """
