...
# Importar CRUDs
# NOTA: crud_mensaje legacy eliminado - usar crud.communication.chat para el sistema moderno
# Importar schemas
...
# NOTA: mensaje_enums legacy eliminado - usar enums del sistema moderno en chat.py si es necesario
...
@router.post(
    "/chatbots/", response_model=ChatBotRead, status_code=status.HTTP_201_CREATED
)
def create_chatbot(*, db: Session = Depends(get_db), chatbot_in: ChatBotCreate) -> Any:
    """Crear nuevo chatbot."""
    ...
    return chat_bot.create(db=db, obj_in=chatbot_in)
...
def read_chatbots(
    ...
    return chat_bot.get_multi(db, skip=skip, limit=limit)


@router.get("/chatbots/{chat_bot_id}", response_model=ChatBotRead)
def read_chatbot(*, db: Session = Depends(get_db), chat_bot_id: UUID) -> Any:
    """Obtener chatbot por ID."""
    chatbot = chat_bot.get(db=db, chat_bot_id=chat_bot_id)
    if not chatbot:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=CHAT_BOT_NOT_FOUND
        )
    return chatbot


@router.get("/chatbots/nombre/{nombre}", response_model=ChatBotRead)
def read_chatbot_by_name(*, db: Session = Depends(get_db), nombre: str) -> Any:
...
def read_active_chatbots(db: Session = Depends(get_db)) -> Any:
...
def read_inactive_chatbots(db: Session = Depends(get_db)) -> Any:
...
def search_chatbots(*, db: Session = Depends(get_db), search_term: str) -> Any:
...
def update_chatbot(
    *, db: Session = Depends(get_db), chat_bot_id: UUID, chatbot_in: ChatBotUpdate
) -> Any:
    """Actualizar chatbot."""
    chatbot = chat_bot.get(db=db, chat_bot_id=chat_bot_id)
    if not chatbot:
    ...
    return chat_bot.update(db=db, db_obj=chatbot, obj_in=chatbot_in)


@router.patch("/chatbots/{chat_bot_id}/activar", response_model=ChatBotRead)
def activate_chatbot(*, db: Session = Depends(get_db), chat_bot_id: UUID) -> Any:
    """Activar chatbot."""
...
def deactivate_chatbot(*, db: Session = Depends(get_db), chat_bot_id: UUID) -> Any:
    """Desactivar chatbot."""
...
def delete_chatbot(*, db: Session = Depends(get_db), chat_bot_id: UUID) -> Any:
    """Eliminar chatbot."""
