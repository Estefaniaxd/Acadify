...
def read_faq_bot(*, db: Session = Depends(get_db), faq_id: UUID) -> Any:
    ...
    if not faq_bot:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="FAQ no encontrada"
        )
    return faq_bot
...
def bulk_update_categoria(
    *,
    ...
    return {
        "message": f"Categoría actualizada exitosamente",
...
def duplicate_faq(
    *,
    db: Session = Depends(get_db),
    faq_id: UUID,
    new_categoria: Optional[str] = Query(
    ...
    new_faq = crud_faqbot.duplicate_faq(
        db=db, faq_id=faq_id, new_categoria=new_categoria
    )
    if not new_faq:
        raise HTTPException(
