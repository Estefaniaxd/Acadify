...
def get_my_insignias(
    ...
    """
    insignias = crud_insignias.get_insignias_usuario(db, current_user.usuario_id, skip=skip, limit=limit)
    total_insignias = crud_insignias.count_insignias_usuario(db, current_user.usuario_id)
    return {
...
def create_insignia(insignia: schemas_insignias.InsigniaCreate, db: Session = Depends(get_db)):
...
def update_insignia(insignia_id: str, insignia_update: schemas_insignias.InsigniaUpdate, db: Session = Depends(get_db)):
