...
def create_recompensa(recompensa: schemas_recompensas.RecompensaCreate, db: Session = Depends(get_db)):
...
def update_recompensa(recompensa_id: str, recompensa_update: schemas_recompensas.RecompensaUpdate, db: Session = Depends(get_db)):
    recompensa = crud_recompensas.update_recompensa(db, recompensa_id, recompensa_update)
    if not recompensa:
...
def get_available_rewards(
    ...
    return crud_recompensas.get_recompensas_disponibles_usuario(db, current_user.usuario_id)
...
def get_my_redeem_history(
    ...
    canjes_data = crud_recompensas.get_estadisticas_canjes_usuario(db, current_user.usuario_id)
    return {
