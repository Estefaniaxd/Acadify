...
def iniciar_intento_examen(
    *,
    ...
    contraseña: str | None = Body(
        None, embed=True, description="Contraseña del examen si es requerida"
    ),
    request: Request,
    ...
    if not acceso_info["puede_acceder"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail=acceso_info["mensaje"]
        )
    ...
    try:
        ...
        return intento_examen.crear_intento(
            db=db,
...
def obtener_intento(
    *,
    ...
    puede_ver = (
        ...
        or intento.estudiante_id == current_user.user_id
    )
...
def obtener_preguntas_intento(
    *,
    ...
    if (
        ...
        and intento.estudiante_id != current_user.user_id
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="No tienes permisos para ver este intento",
        ...
        for pregunta_id in intento.orden_preguntas:
            ...
            if pregunta:
                ...
                pregunta_dict = {
                    ...
                    "orden": len(preguntas_ordenadas)
                    + 1,  # Orden relativo en el intento
...
def guardar_respuesta(
    *,
    ...
    respuesta_datos: dict[str, Any] = Body(
        ..., embed=True, description="Datos de la respuesta"
    ),
    tiempo_empleado: int | None = Body(
        None, embed=True, description="Tiempo empleado en segundos"
    ),
    current_user: Usuario = Depends(get_current_user),
):
    ...
    if (
        current_user.role != "estudiante"
        or intento.estudiante_id != current_user.user_id
        or intento.estado != EstadoIntento.EN_PROGRESO
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def avanzar_pregunta(
    *,
    db: Session = Depends(get_db),
    intento_id: str = Path(..., description="ID del intento"),
    nueva_pregunta: int | None = Body(
        None, embed=True, description="Número de pregunta específica"
    ),
    current_user: Usuario = Depends(get_current_user),
):
    ...
    return intento_examen.avanzar_pregunta(
...
def finalizar_intento(
    *,
    ...
    puede_finalizar = (
        current_user.role == "estudiante"
        and intento.estudiante_id == current_user.user_id
    ) or current_user.role in ["coordinador", "administrador"]
    ...
    return intento_examen.finalizar_intento(
    ...
    *,
    ...
    if current_user.role == "estudiante":
        ...
        mostrar_respuestas_correctas = (
            mostrar_respuestas_correctas and exam.mostrar_respuestas_correctas
        )

    # Obtener estadísticas del intento
    estadisticas_intento = intento_examen.get_estadisticas_intento(
        db=db, intento_id=intento_id
    )
...
def obtener_estadisticas_intento(
    *,
    ...
    return intento_examen.get_estadisticas_intento(db=db, intento_id=intento_id)
...
def obtener_mis_intentos(
    *,
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(get_current_user),
    examen_id: str | None = Query(None, description="Filtrar por examen específico"),
    ...
    return intento_examen.get_intentos_por_estudiante(
        db=db,
        estudiante_id=current_user.user_id,
        examen_id=examen_id,
        skip=skip,
...
def obtener_intentos_examen(
    *,
    ...
    examen_id: str = Path(..., description="ID del examen"),
    current_user: Usuario = Depends(get_current_user),
    estado: EstadoIntento | None = Query(None, description="Filtrar por estado"),
    skip: int = Query(0, ge=0),
    limit: int = Query(100, ge=1, le=200),
):
    ...
    # Verificar permisos
    puede_ver_intentos = current_user.role in ["coordinador", "administrador"] or (
    ...
    return intento_examen.get_intentos_por_examen(
...
def obtener_ranking_examen(
    *,
    ...
    current_user: Usuario = Depends(get_current_user),
    limite: int = Query(
        10, ge=1, le=50, description="Número de posiciones del ranking"
    ),
):
    ...
    # Verificar permisos
    puede_ver_ranking = current_user.role in ["coordinador", "administrador"] or (
        current_user.role == "docente" and exam.creado_por == current_user.user_id
    )
    ...
    return intento_examen.get_ranking_intentos(
        db=db, examen_id=examen_id, limite=limite
    )


@router.get(
    "/{estudiante_id}/recuperar/{examen_id}",
    response_model=Optional[IntentoExamenResponse],
)
def recuperar_intento_interrumpido(
    *,
    ...
    return intento_examen.recuperar_intento(
