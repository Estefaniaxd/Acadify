...
from src.models.evaluaciones.examen import DificultadPregunta, TipoPregunta
...
@router.post(
    "/", response_model=BancoPreguntaResponse, status_code=status.HTTP_201_CREATED
)
def crear_pregunta_banco(
    *,
    ...
    return banco_pregunta.create(db=db, obj_in=pregunta_in)
...
def listar_preguntas_banco(
    *,
    ...
    # Filtros de búsqueda
    tipo_pregunta: TipoPregunta | None = Query(
        None, description="Filtrar por tipo de pregunta"
    ),
    dificultad: DificultadPregunta | None = Query(
        None, description="Filtrar por dificultad"
    ),
    materia: str | None = Query(None, description="Filtrar por materia"),
    tema: str | None = Query(None, description="Filtrar por tema"),
    categoria: str | None = Query(None, description="Filtrar por categoría"),
    nivel_educativo: str | None = Query(
        None, description="Filtrar por nivel educativo"
    ),
    es_publica: bool | None = Query(None, description="Filtrar por visibilidad"),
    texto_busqueda: str | None = Query(None, description="Búsqueda por texto"),
    ...
    return banco_pregunta.buscar_con_filtros(
        db=db,
        filtros=filtros,
...
def obtener_pregunta_banco(
    *,
    ...
    puede_ver = (
        ...
        or pregunta.creado_por == current_user.user_id
        or pregunta.es_publica
        or (institucion_id and pregunta.institucion_id == institucion_id)
    )
...
def actualizar_pregunta_banco(
    *,
    ...
    puede_editar = (
        ...
        or pregunta.creado_por == current_user.user_id
        or (
            current_user.role == "coordinador"
            and institucion_id
            and pregunta.institucion_id == institucion_id
        )
    )
    ...
    return banco_pregunta.update(db=db, db_obj=pregunta, obj_in=pregunta_in)
...
def revisar_pregunta(
    *,
    ...
    comentarios: str | None = Body(
        None, embed=True, description="Comentarios de la revisión"
    ),
    current_user: Usuario = Depends(get_current_user),
):
...
def obtener_preguntas_pendientes_revision(
    *,
    ...
    return banco_pregunta.get_preguntas_pendientes_revision(
...
def obtener_materias_disponibles(
    *, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)
    ...
    institucion_id = getattr(current_user, "institucion_id", None)
    return banco_pregunta.get_materias_disponibles(
...
def obtener_temas_por_materia(
    *,
    ...
    institucion_id = getattr(current_user, "institucion_id", None)
    return banco_pregunta.get_temas_por_materia(
        db=db,
        materia=materia,
        usuario_id=current_user.user_id,
        institucion_id=institucion_id,
...
def obtener_tags_populares(
    *,
    ...
    institucion_id = getattr(current_user, "institucion_id", None)
    return banco_pregunta.get_tags_populares(
        db=db,
        limite=limite,
        usuario_id=current_user.user_id,
        institucion_id=institucion_id,
...
def obtener_estadisticas_banco(
    *, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)
    ...
    institucion_id = getattr(current_user, "institucion_id", None)
    return banco_pregunta.get_estadisticas_banco(
        db=db, usuario_id=current_user.user_id, institucion_id=institucion_id
    )
...
def exportar_preguntas(
    *,
    db: Session = Depends(get_db),
    pregunta_ids: list[str] = Body(
        ..., description="Lista de IDs de preguntas a exportar"
    ),
    ...
    return banco_pregunta.exportar_preguntas(
...
async def importar_preguntas(
    *,
    ...
    try:
        ...
        # Importar preguntas
        return banco_pregunta.importar_preguntas(
            db=db,
        ...
        raise HTTPException(
            ...
            detail=f"Error al procesar el archivo: {e!s}",
        ) from e
