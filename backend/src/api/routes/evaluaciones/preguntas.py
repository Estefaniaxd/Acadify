...
@router.post(
    "/", response_model=PreguntaExamenResponse, status_code=status.HTTP_201_CREATED
)
def crear_pregunta(
    *,
    ...
    created_pregunta = pregunta_examen.create_pregunta_con_orden(
        db=db, obj_in=pregunta_in
    )
...
def listar_preguntas_examen(
    *,
    ...
    current_user: Usuario = Depends(get_current_user),
    para_estudiante: bool = Query(
        False, description="Filtrar información sensible para estudiantes"
    ),
):
    ...
    puede_ver = (
        ...
        or (current_user.role == "docente" and exam.creado_por == current_user.user_id)
        or current_user.role == "estudiante"
    )
...
def obtener_pregunta(
    *,
    ...
    puede_ver = current_user.role in ["coordinador", "administrador"] or (
        current_user.role == "docente" and exam.creado_por == current_user.user_id
    )
...
def actualizar_pregunta(
    *,
    ...
    updated_pregunta = pregunta_examen.update(
        db=db, db_obj=pregunta, obj_in=pregunta_in
    )
...
def eliminar_pregunta(
    *,
    ...
    # Verificar permisos
    exam = examen.get(db=db, id=pregunta.examen_id)
    puede_eliminar = current_user.role in ["coordinador", "administrador"] or (
        current_user.role == "docente" and exam.creado_por == current_user.user_id
    )
    ...
    success = pregunta_examen.eliminar_con_reordenamiento(
        db=db, pregunta_id=pregunta_id
    )
...
def reordenar_preguntas(
    *,
    db: Session = Depends(get_db),
    examen_id: str = Path(..., description="ID del examen"),
    nuevos_ordenes: list[dict[str, Any]] = Body(
        ..., description="Lista con nuevos órdenes"
    ),
    current_user: Usuario = Depends(get_current_user),
):
    ...
    # Verificar permisos
    puede_reordenar = current_user.role in ["coordinador", "administrador"] or (
        current_user.role == "docente" and exam.creado_por == current_user.user_id
    )
    ...
    try:
        return pregunta_examen.reordenar_preguntas(
            db=db, examen_id=examen_id, nuevos_ordenes=nuevos_ordenes
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error al reordenar preguntas: {e!s}",
        ) from e
...
def importar_pregunta_desde_banco(
    *,
    db: Session = Depends(get_db),
    examen_id: str = Path(..., description="ID del examen"),
    banco_pregunta_id: str = Body(
        ..., embed=True, description="ID de la pregunta en el banco"
    ),
    puntuacion_personalizada: float | None = Body(
        None, embed=True, description="Puntuación personalizada"
    ),
    orden_personalizado: int | None = Body(
        None, embed=True, description="Orden personalizado"
    ),
    current_user: Usuario = Depends(get_current_user),
):
    ...
    # Verificar permisos
    puede_importar = current_user.role in ["coordinador", "administrador"] or (
        current_user.role == "docente" and exam.creado_por == current_user.user_id
    )
...
def duplicar_pregunta(
    *,
    ...
    nuevo_examen_id: str | None = Body(
        None, embed=True, description="ID del examen destino (opcional)"
    ),
    current_user: Usuario = Depends(get_current_user),
):
    ...
    puede_duplicar = current_user.role in ["coordinador", "administrador"] or (
        current_user.role == "docente"
        and exam_original.creado_por == current_user.user_id
    )
    ...
    if nuevo_examen_id and nuevo_examen_id != pregunta_original.examen_id:
        ...
        puede_editar_destino = current_user.role in [
            "coordinador",
            "administrador",
        ] or (
            current_user.role == "docente"
            and exam_destino.creado_por == current_user.user_id
        )
...
def obtener_estadisticas_pregunta(
    *,
    ...
    return pregunta_examen.get_estadisticas_pregunta(db=db, pregunta_id=pregunta_id)
...
def validar_pregunta(
    *,
    ...
    # Verificar permisos
    exam = examen.get(db=db, id=pregunta.examen_id)
    puede_validar = current_user.role in ["coordinador", "administrador"] or (
        current_user.role == "docente" and exam.creado_por == current_user.user_id
    )
    ...
    return pregunta_examen.validar_pregunta(db=db, pregunta_id=pregunta_id)
