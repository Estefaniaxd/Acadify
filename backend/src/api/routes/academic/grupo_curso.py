...
from src.schemas.academic.grupo_curso import (
    GrupoCurso,
    GrupoCursoCreate,
    GrupoCursoUpdate,
)
...
def get_all(db: Session = Depends(get_db)):
    return grupo_curso.get_multi(db)


@router.get("/{grupo_curso_id}", response_model=GrupoCurso)
def get_one(grupo_curso_id: UUID, db: Session = Depends(get_db)):
    obj = grupo_curso.get(db, grupo_curso_id)
    if not obj:
        raise HTTPException(
            status_code=404, detail="Relación Grupo-Curso no encontrada"
        )
    return obj
...
def create(obj_in: GrupoCursoCreate, db: Session = Depends(get_db)):
    return grupo_curso.create(db, obj_in)


@router.put("/{grupo_curso_id}", response_model=GrupoCurso)
def update(
    grupo_curso_id: UUID, obj_in: GrupoCursoUpdate, db: Session = Depends(get_db)
):
    db_obj = grupo_curso.get(db, grupo_curso_id)
    if not db_obj:
        raise HTTPException(
            status_code=404, detail="Relación Grupo-Curso no encontrada"
        )
    return grupo_curso.update(db, db_obj, obj_in)


@router.delete("/{grupo_curso_id}", response_model=GrupoCurso)
def delete(grupo_curso_id: UUID, db: Session = Depends(get_db)):
    obj = grupo_curso.remove(db, grupo_curso_id)
    if not obj:
        raise HTTPException(
            status_code=404, detail="Relación Grupo-Curso no encontrada"
        )
