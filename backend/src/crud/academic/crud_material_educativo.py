...
from src.enums.academic.material_educativo_enums import (
    CarpetaMaterial,
    EstadoMaterial,
    TipoMaterialEducativo,
)
from src.models.academic.material_educativo import MaterialEducativo
from src.schemas.academic.material_educativo import (
...
class CRUDMaterialEducativo(
    ...
    def get(self, db: Session, material_id: UUID) -> MaterialEducativo | None:
    ...
    def get_multi_by_carpeta(
        ...
        return (
            query.filter(MaterialEducativo.es_version_actual)
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_multi_by_autor(
        self, db: Session, autor_id: UUID, skip: int = 0, limit: int = 100
    ) -> list[MaterialEducativo]:
        ...
        return (
            ...
            .filter(
                and_(
                    ...
                    MaterialEducativo.es_version_actual,
                )
            )
            .offset(skip)
            .limit(limit)
    ...
    def subir_nueva_version(
        ...
        if not material_original:
            msg = "Material original no encontrado"
        ...
        ultima_version = (
            db.query(func.max(MaterialEducativo.version))
            .filter(MaterialEducativo.material_original_id == material_id)
            .scalar()
            or material_original.version
        )

        nueva_version_numero = ultima_version + 1

        # Calcular hash
        hash_archivo = nueva_version.hash_archivo or self._calcular_hash_archivo(
            nueva_version.url_archivo
        )
    ...
    def buscar_material(
        ...
        query = query.filter(MaterialEducativo.es_version_actual)
        query = query.filter(MaterialEducativo.estado == EstadoMaterial.activo)
    ...
    def _calcular_hash_archivo(self, url_archivo: str) -> str | None:
...
material_educativo_crud = CRUDMaterialEducativo(
    MaterialEducativo, id_field="material_id"
)
