...
class CRUDMaterialClase:
    def get(self, db: Session, material_clase_id: UUID):
        return db.query(MaterialClase).filter(MaterialClase.material_clase_id == material_clase_id).first()

    def get_all(self, db: Session, skip: int = 0, limit: int = 100):
        return db.query(MaterialClase).offset(skip).limit(limit).all()

    def create(self, db: Session, obj_in: MaterialClaseCreate):
        db_obj = MaterialClase(**obj_in.dict())
        db.add(db_obj)
        db.commit()
        db.refresh(db_obj)
        return db_obj
...
crud_material_clase = CRUDMaterialClase()
