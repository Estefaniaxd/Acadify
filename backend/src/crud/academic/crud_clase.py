...
from src.enums.academic.clase_enums import EstadoClase, EstadoCodigoAcceso
from src.models.academic.clase import Clase, HistorialAccesoClase
from src.schemas.academic.clase import ClaseCreate, ClaseUpdate
...
class CRUDClase(CRUDBase[Clase, ClaseCreate, ClaseUpdate]):
    ...
    def get(self, db: Session, clase_id: UUID) -> Clase | None:
        ...
        return (
            ...
            .filter(Clase.clase_id == clase_id)
            .first()
        )
    ...
    def update(self, db: Session, db_obj: Clase, obj_in: ClaseUpdate) -> Clase:
        ...
        db_obj.fecha_actualizacion = datetime.now(UTC)
        db.add(db_obj)
        db.commit()
        db.refresh(db_obj)
        return db_obj
    ...
    def get_by_codigo_acceso(self, db: Session, codigo: str) -> Clase | None:
            ...
            .filter(
                and_(
                    ...
                    or_(
                        ...
                        Clase.fecha_vencimiento_codigo > datetime.now(UTC),
                    ),
                )
            )
            .first()
    ...
    def unirse_a_clase(
        ...
        codigo: str,
        estudiante_id: UUID,
        ip_acceso: str | None = None,
        user_agent: str | None = None,
    ) -> tuple[bool, str, Clase | None]:
        ...
        if clase.max_estudiantes:
            total_unidos = (
                db.query(HistorialAccesoClase)
                .filter(HistorialAccesoClase.clase_id == clase.clase_id)
                .count()
            )
            ...
            if total_unidos >= clase.max_estudiantes:
                return (
                    False,
                    "La clase ha alcanzado el límite máximo de estudiantes",
                    clase,
                )
    ...
    def get_historial_accesos(
        self, db: Session, clase_id: UUID
    ) -> list[HistorialAccesoClase]:
        """Obtiene el historial de accesos de una clase."""
        return (
            db.query(HistorialAccesoClase)
            .options(selectinload(HistorialAccesoClase.estudiante))
            .filter(HistorialAccesoClase.clase_id == clase_id)
            .order_by(HistorialAccesoClase.fecha_acceso.desc())
            .all()
        )
    ...
    def get_estadisticas_clase(self, db: Session, clase_id: UUID) -> dict:
        ...
        return {
            ...
            "fecha_ultima_union": db.query(func.max(HistorialAccesoClase.fecha_acceso))
            .filter(HistorialAccesoClase.clase_id == clase_id)
            .scalar(),
        }
    ...
    def _generar_codigo_unico(self, db: Session) -> str:
        ...
        msg = "No se pudo generar un código único después de varios intentos"
        raise ValueError(msg)

    def obtener_clases_proximas(
        self, db: Session, docente_id: UUID | None = None, limite_horas: int = 24
    ) -> list[Clase]:
        """Obtiene clases programadas en las próximas horas."""
        fecha_limite = datetime.now(UTC) + timedelta(hours=limite_horas)

        query = db.query(Clase).filter(
            and_(
                Clase.fecha_inicio >= datetime.now(UTC),
