...
class CRUDAvatarAsset(CRUDBase[AvatarAsset, dict[str, Any], dict[str, Any]]):
    ...
    def get_by_filename(self, db: Session, *, filename: str) -> AvatarAsset | None:
    ...
    def get_assets_by_categories(
        self, db: Session, *, categories: list[str], active_only: bool = True
    ...
    def update_asset_status(
        self, db: Session, *, asset_id: str, is_active: bool
    ) -> AvatarAsset | None:
    ...
    def validate_assets_exist(
        self, db: Session, *, asset_files: list[str]
    ) -> dict[str, bool]:
        ...
        """
        existing_assets = (
            db.query(AvatarAsset.filename)
            .filter(
                and_(
                    AvatarAsset.filename.in_(asset_files), AvatarAsset.is_active == "Y"
                )
            )
            .all()
        )
        ...
        return {filename: filename in existing_filenames for filename in asset_files}
    ...
    def get_stats(self, db: Session) -> dict[str, Any]:
        ...
        active_assets = (
            db.query(AvatarAsset).filter(AvatarAsset.is_active == "Y").count()
        )
        ...
        for category in categories:
            total_cat = (
                db.query(AvatarAsset).filter(AvatarAsset.category == category).count()
            )
            active_cat = (
                db.query(AvatarAsset)
                .filter(
                    and_(AvatarAsset.category == category, AvatarAsset.is_active == "Y")
                )
                .count()
            )
            category_stats[category] = {"total": total_cat, "active": active_cat}
    ...
    def get_base_assets(self, db: Session) -> dict[str, AvatarAsset]:
        """Obtiene los assets base por género.
        ...
        base_assets = (
            db.query(AvatarAsset)
            .filter(and_(AvatarAsset.category == "base", AvatarAsset.is_active == "Y"))
            .all()
        )

        result = {}
        for asset in base_assets:
            # Usar el campo target_gender en lugar de buscar en el filename
            if asset.target_gender == "male":
                result["male"] = asset
            elif asset.target_gender == "female":
                result["female"] = asset
