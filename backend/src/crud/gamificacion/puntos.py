...
from src.schemas.gamificacion.puntos import (
    HistorialPuntosCreate,
...
def get_usuario_puntos(db: Session, usuario_id: uuid.UUID) -> Optional[UsuarioPuntos]:
    ...
    return db.query(UsuarioPuntos).filter(
        UsuarioPuntos.usuario_id == usuario_id
    ).first()


def create_usuario_puntos(db: Session, usuario_id: uuid.UUID, puntos_iniciales: int = 0) -> UsuarioPuntos:
...
def get_historial_puntos_usuario(
    ...
    ).order_by(desc(HistorialPuntos.fecha)).offset(skip).limit(limit).all()
...
def get_ranking_usuarios(
    ...
    query = db.query(
        ...
        Usuario, UsuarioPuntos.usuario_id == Usuario.usuario_id
    ).filter(
        UsuarioPuntos.puntos_acumulados > 0
    ).order_by(
        desc(UsuarioPuntos.puntos_acumulados)
...
def get_posicion_usuario_ranking(db: Session, usuario_id: uuid.UUID) -> Optional[int]:
    ...
    posicion = db.query(
        func.count(UsuarioPuntos.usuario_id).label('posicion')
    ).filter(
