...
def get_temas_predefinidos(db: Session) -> List[Tema]:
    ...
    return db.query(Tema).join(
        TemaPredefinido, Tema.tema_id == TemaPredefinido.tema_id
    ).order_by(Tema.nombre).all()
...
def create_tema_predefinido(db: Session, tema_id: uuid.UUID) -> TemaPredefinido:
    ...
    existing = db.query(TemaPredefinido).filter(
        TemaPredefinido.tema_id == tema_id
    ).first()
...
def remove_tema_predefinido(db: Session, tema_id: uuid.UUID) -> bool:
    ...
    tema_predefinido = db.query(TemaPredefinido).filter(
        TemaPredefinido.tema_id == tema_id
    ).first()
...
def get_temas_personalizados_usuario(
    ...
    return db.query(TemaPersonalizado).options(
        joinedload(TemaPersonalizado.tema)
    ).filter(
...
def asignar_tema_personalizado(
    db: Session,
    request: AsignarTemaPersonalizadoRequest
) -> TemaPersonalizado:
    ...
    existing = db.query(TemaPersonalizado).filter(
    ...
    es_predefinido = db.query(TemaPredefinido).filter(
        TemaPredefinido.tema_id == request.tema_id
    ).first()
...
def revocar_tema_personalizado(
    ...
    tema_personalizado = db.query(TemaPersonalizado).filter(
        TemaPersonalizado.usuario_id == usuario_id,
...
def is_tema_predefinido(db: Session, tema_id: uuid.UUID) -> bool:
    ...
    return db.query(TemaPredefinido).filter(
        TemaPredefinido.tema_id == tema_id
    ).first() is not None
...
def count_usuarios_con_tema(db: Session, tema_id: uuid.UUID) -> int:
    ...
    return db.query(TemaPersonalizado).filter(
        TemaPersonalizado.tema_id == tema_id
    ).count()