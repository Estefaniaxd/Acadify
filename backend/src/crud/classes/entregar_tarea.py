...
from src.schemas.classes.entregar_tarea import EntregarTareaCreate, EntregarTareaUpdate


class CRUDEntregarTarea:
    def create(self, db: Session, *, obj_in: EntregarTareaCreate) -> EntregarTarea:
        """Crear nueva entrega de tarea"""
        db_obj = EntregarTarea(**obj_in.dict())
        db.add(db_obj)
        db.commit()
        db.refresh(db_obj)
        return db_obj
    ...
    def get(self, db: Session, entrega_id: UUID) -> Optional[EntregarTarea]:
        ...
        return db.query(EntregarTarea).filter(EntregarTarea.entrega_id == entrega_id).first()
    ...
    def get_by_estudiante(self, db: Session, estudiante_id: UUID) -> List[EntregarTarea]:
        """Obtener entregas por estudiante"""
        return db.query(EntregarTarea).filter(EntregarTarea.estudiante_id == estudiante_id).all()
    ...
    def remove(self, db: Session, *, entrega_id: UUID) -> EntregarTarea:
        ...
        obj = db.query(EntregarTarea).filter(EntregarTarea.entrega_id == entrega_id).first()
        if obj:
            db.delete(obj)
            db.commit()
        return obj