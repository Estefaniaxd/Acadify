...
def get_insignias_usuario(
    ...
    return (
        db.query(UsuarioInsignia)
        .options(joinedload(UsuarioInsignia.insignia))
        .filter(UsuarioInsignia.usuario_id == usuario_id)
        .order_by(desc(UsuarioInsignia.fecha_otorgada))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def get_insignias_con_estadisticas(db: Session) -> List[dict]:
    ...
    query = (
        db.query(
            Insignia, func.count(UsuarioInsignia.usuario_id).label("total_usuarios")
        )
        .outerjoin(UsuarioInsignia, Insignia.insignia_id == UsuarioInsignia.insignia_id)
        .group_by(Insignia.insignia_id)
    ...
    for insignia, total_usuarios_con_insignia in query.all():
        porcentaje = (
            (total_usuarios_con_insignia / total_usuarios * 100)
            if total_usuarios > 0
            else 0
        )
        results.append(
...
def get_usuarios_con_insignia(
    ...
    return (
        db.query(UsuarioInsignia)
        .options(joinedload(UsuarioInsignia.usuario))
        .filter(UsuarioInsignia.insignia_id == insignia_id)
        .order_by(desc(UsuarioInsignia.fecha_otorgada))
        .offset(skip)
        .limit(limit)
        .all()
