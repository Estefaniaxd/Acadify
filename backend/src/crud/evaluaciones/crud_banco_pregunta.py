...
class CRUDBancoPregunta(
    CRUDBase[BancoPregunta, BancoPreguntaCreate, BancoPreguntaUpdate]
):

    def get_preguntas_por_profesor(
        self, db: Session, profesor_id: str, skip: int = 0, limit: int = 100
    ) -> list[BancoPregunta]:
        """Obtener preguntas del banco creadas por un profesor específico."""
        return (
            db.query(BancoPregunta)
            .filter(BancoPregunta.creado_por == profesor_id)
            .order_by(desc(BancoPregunta.fecha_creacion))
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_preguntas_publicas(
        ...
        return (
            db.query(BancoPregunta)
            .filter(BancoPregunta.es_publica)
            .order_by(desc(BancoPregunta.veces_utilizada))
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_preguntas_institucion(
        ...
        return (
            query.order_by(desc(BancoPregunta.fecha_creacion))
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def buscar_con_filtros(
        ...
        if usuario_id and institucion_id:
            query = query.filter(
                or_(
                    ...
                    BancoPregunta.es_publica,  # Públicas
        ...
        if filtros.categoria:
            query = query.filter(
                BancoPregunta.categoria.ilike(f"%{filtros.categoria}%")
            )

        # Filtro por nivel educativo
        if filtros.nivel_educativo:
            query = query.filter(
                BancoPregunta.nivel_educativo == filtros.nivel_educativo
            )
        ...
        query = query.order_by(
            desc(BancoPregunta.veces_utilizada), desc(BancoPregunta.fecha_creacion)
        )
    ...
    def get_temas_por_materia(
        ...
        query = (
            db.query(BancoPregunta.tema)
            .distinct()
            .filter(BancoPregunta.materia == materia)
        )
    ...
    def get_tags_populares(
        ...
        return [{"tag": tag, "count": count} for tag, count in tags_ordenados[:limite]]
    ...
    def duplicar_pregunta(
        ...
        nueva_institucion_id: str | None = None,
    ) -> BancoPregunta | None:
    ...
    def marcar_como_publica(
        self, db: Session, pregunta_id: str, usuario_id: str
    ) -> BancoPregunta | None:
    ...
    def get_preguntas_pendientes_revision(
        ...
        return (
            query.order_by(asc(BancoPregunta.fecha_actualizacion))
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_estadisticas_banco(
        ...
        if usuario_id:
            ...
            tipos_propias = (
                db.query(
                    BancoPregunta.tipo_pregunta, func.count(BancoPregunta.pregunta_id)
                )
                .filter(BancoPregunta.creado_por == usuario_id)
                .group_by(BancoPregunta.tipo_pregunta)
                .all()
            )
            ...
            dificultades_propias = (
                db.query(
                    BancoPregunta.dificultad, func.count(BancoPregunta.pregunta_id)
                )
                .filter(BancoPregunta.creado_por == usuario_id)
                .group_by(BancoPregunta.dificultad)
                .all()
            )

            # Preguntas más utilizadas (propias)
            mas_utilizadas_propias = (
                query_usuario.order_by(desc(BancoPregunta.veces_utilizada))
                .limit(5)
                .all()
            )
        ...
        total_publicas = query_base.filter(BancoPregunta.es_publica).count()
        ...
        tipos_accesibles = (
            db.query(BancoPregunta.tipo_pregunta, func.count(BancoPregunta.pregunta_id))
            .filter(query_accesibles.statement.whereclause)
            .group_by(BancoPregunta.tipo_pregunta)
            .all()
        )
    ...
    def actualizar_estadisticas_uso(
        ...
        db: Session,
        pregunta_id: str,
        promedio_aciertos: float | None = None,
    ...
    def exportar_preguntas(
        self, db: Session, pregunta_ids: list[str], formato: str = "json"
    ) -> dict[str, Any]:
        """Exportar preguntas seleccionadas."""
        preguntas = (
            db.query(BancoPregunta)
            .filter(BancoPregunta.pregunta_id.in_(pregunta_ids))
            .all()
        )
            ...
            try:
                pregunta_create = BancoPreguntaCreate(
                    ...
                    dificultad=DificultadPregunta(
                        pregunta_data.get("dificultad", "medio")
                    ),
                    ...
                    tiempo_estimado_segundos=pregunta_data.get(
                ...
                errores.append(f"Error en pregunta {i+1}: {e!s}")
