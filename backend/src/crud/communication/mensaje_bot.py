...
class CRUDMensajeBot(CRUDMensajeBot[MensajeBot, MensajeBotCreate, MensajeBotUpdate]):
    ...
    def get(self, db: Session, mensaje_bot_id: UUID) -> Optional[MensajeBot]:
        """Obtener mensaje bot por ID"""
        return db.query(MensajeBot).filter(MensajeBot.mensaje_bot_id == mensaje_bot_id).first()
    ...
    def get_by_material_educativo(self, db: Session, material_id: UUID) -> List[MensajeBot]:
    ...
    def get_by_contexto(self, db: Session, contexto: ContextoMensaje) -> List[MensajeBot]:
    ...
    def remove(self, db: Session, *, mensaje_bot_id: UUID) -> Optional[MensajeBot]:
        """Eliminar mensaje bot"""
        obj = db.query(MensajeBot).filter(MensajeBot.mensaje_bot_id == mensaje_bot_id).first()
        if obj:
            db.delete(obj)
            db.commit()
        return obj
    ...
    def remove_old_messages(self, db: Session, *, older_than: datetime) -> int:
        ...
        deleted_count = (
            db.query(MensajeBot)
            .filter(MensajeBot.fecha_hora < older_than)
            .delete()
        )
        db.commit()
