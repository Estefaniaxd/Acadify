...
class CRUDSalaChat(CRUDBase[SalaChat, SalaChatCreate, SalaChatUpdate]):
    ...
    def get_estadisticas_sala(self, db: Session, *, sala_id: str) -> dict[str, Any]:
        ...
        # Total de mensajes
        total_mensajes = (
            db.query(func.count(Mensaje.id)).filter(Mensaje.sala_id == sala_id).scalar()
        ) or 0
        ...
        mensajes_hoy = (
            ...
            .filter(
                Mensaje.sala_id == sala_id, func.date(Mensaje.fecha_creacion) == hoy
            )
            .scalar()
        ) or 0
...
class CRUDParticipanteSala(
    CRUDBase[ParticipanteSala, ParticipanteSalaCreate, ParticipanteSalaUpdate]
):
    ...
    def get_participantes_sala(
        ...
        if not incluir_inactivos:
            query = query.filter(self.model.esta_activo)

        return query.all()

    def es_participante(self, db: Session, *, sala_id: str, usuario_id: str) -> bool:
    ...
    def get_participante(
        self, db: Session, *, sala_id: str, usuario_id: str
    ) -> ParticipanteSala | None:
        """Obtener participante específico."""
        return (
            db.query(self.model)
            .filter(self.model.sala_id == sala_id, self.model.usuario_id == usuario_id)
            .first()
        )
    ...
    def actualizar_ultima_lectura(
        ...
        db.query(self.model).filter(
            self.model.sala_id == sala_id, self.model.usuario_id == usuario_id
        ).update({"fecha_ultima_lectura": datetime.now(UTC)})
        db.commit()
...
class CRUDMensaje(CRUDBase[Mensaje, MensajeCreate, MensajeUpdate]):
    ...
    def create_mensaje(
        ...
        if (
            contenido_procesado["menciones_usuarios"]
            or contenido_procesado["menciones_todos"]
        ):
    ...
    def get_mensajes_sala(
        ...
        if not crud_participante.es_participante(
            db, sala_id=sala_id, usuario_id=usuario_id
        ):
            return []
        ...
        if filtros.solo_importantes:
            query = query.filter(self.model.es_importante)

        if filtros.solo_anuncios:
            query = query.filter(self.model.es_anuncio)
        ...
        if filtros.menciona_ia:
            query = query.filter(self.model.menciona_ia)
    ...
    def _actualizar_contador_respuestas(
        self, db: Session, mensaje_padre_id: str
    ) -> None:
        """Actualizar contador de respuestas del mensaje padre."""
        mensaje_padre = (
            db.query(self.model).filter(self.model.id == mensaje_padre_id).first()
        )
        if mensaje_padre:
...
class CRUDLecturaMensaje(CRUDBase[LecturaMensaje, None, None]):
    ...
    def marcar_como_leido(
        ...
        lectura_existente = (
            db.query(self.model)
            .filter(
                self.model.mensaje_id == mensaje_id, self.model.usuario_id == usuario_id
            )
            .first()
        )
...
class CRUDNotificacion(CRUDBase[Notificacion, NotificacionCreate, None]):
    ...
    def get_notificaciones_usuario(
        ...
        if filtros.tipo_notificacion:
            query = query.filter(
                self.model.tipo_notificacion == filtros.tipo_notificacion
            )

        if filtros.solo_no_leidas:
            query = query.filter(not self.model.leida)
    ...
    def marcar_como_leida(
        ...
        resultado = (
            db.query(self.model)
            .filter(
                self.model.id == notificacion_id, self.model.usuario_id == usuario_id
            )
            .update({"leida": True, "fecha_lectura": datetime.now(UTC)})
        )
        db.commit()
        return resultado > 0

    def marcar_todas_leidas(
        self, db: Session, *, usuario_id: str, tipo_notificacion: str | None = None
    ) -> int:
        """Marcar todas las notificaciones como leídas."""
        query = db.query(self.model).filter(
            self.model.usuario_id == usuario_id, not self.model.leida
        )

        if tipo_notificacion:
            query = query.filter(self.model.tipo_notificacion == tipo_notificacion)

        resultado = query.update({"leida": True, "fecha_lectura": datetime.now(UTC)})
        db.commit()
        return resultado
    ...
    def get_count_no_leidas(self, db: Session, *, usuario_id: str) -> int:
        ...
        return (
            db.query(func.count(self.model.id))
            .filter(self.model.usuario_id == usuario_id, not self.model.leida)
            .scalar()
        ) or 0
...
class CRUDConfiguracionNotificaciones(
    CRUDBase[
        ConfiguracionNotificaciones,
        ConfiguracionNotificacionesUpdate,
        ConfiguracionNotificacionesUpdate,
    ...
    def get_by_usuario(
        ...
        config = (
            db.query(self.model).filter(self.model.usuario_id == usuario_id).first()
        )
    ...
    def update_by_usuario(
        ...
        config.fecha_actualizacion = datetime.now(UTC)
        db.commit()
        db.refresh(config)
...
crud_config_notificaciones = CRUDConfiguracionNotificaciones(
    ConfiguracionNotificaciones
)
