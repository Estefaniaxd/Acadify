...
from sqlalchemy.dialects.postgresql import BIGINT, ENUM, INTEGER, TEXT, TIMESTAMP, UUID
...
class MaterialEducativo(Base):
    ...
    material_clases = relationship(
        "MaterialClase", backref="material_educativo", cascade="all, delete-orphan"
    )
    material_cursos = relationship(
        "MaterialCurso", backref="material_educativo", cascade="all, delete-orphan"
    )
