...
class Programa(Base):
    ...
    duracion_periodos = Column(
        Integer, nullable=True
    )  # Número de períodos (ej: 10 semestres)
    ...
    puntaje_minimo_admision = Column(
        DECIMAL(5, 2), nullable=True
    )  # Puntaje de examen de estado
    ...
    competencias_desarrolladas = Column(
        JSON, nullable=True
    ...
    def duracion_total_meses(self) -> int:
        ...
        if self.duracion_periodos and self.duracion_tipo:
            ...
            return self.duracion_periodos * meses_por_periodo.get(
                self.duracion_tipo.value, 0
            )
