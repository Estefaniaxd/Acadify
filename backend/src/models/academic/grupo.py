...
from sqlalchemy import Column, text, String, ForeignKey, Boolean, Integer, DECIMAL, TIME, DATE
from sqlalchemy.dialects.postgresql import UUID, ENUM, JSON, TEXT, TIMESTAMP
from ...enums.academic.grupo_enums import JornadaGrupo, EstadoGrupo, TipoGrupo, ModalidadAsistencia, FormatoEvaluacion
...
class Grupo(Base):
    ...
    tipo_grupo = Column(
        ENUM(TipoGrupo, name="tipo_grupo", create_type=False),
        nullable=True
    )
    ...
    dias_semana = Column(JSON, nullable=True)  # Lista de días (ej: ["lunes", "miércoles"])
    ...
    creado_por_id = Column(UUID(as_uuid=True), ForeignKey("Usuario.usuario_id", ondelete="SET NULL"), nullable=True)
    modificado_por_id = Column(UUID(as_uuid=True), ForeignKey("Usuario.usuario_id", ondelete="SET NULL"), nullable=True)
