...
from sqlalchemy import Column, text, ForeignKey, String, UniqueConstraint, Index, Boolean, DECIMAL
from sqlalchemy.dialects.postgresql import UUID, ENUM, DATE, TEXT, TIMESTAMP, INTEGER, JSON
from ...enums.academic.curso_enums import ModalidadCurso, NivelDificultad, TipoCurso, CategoriaCurso, EstadoCurso, IdiomaCurso
...
class Curso(Base):
    ...
    nivel_dificultad = Column(
        ENUM(NivelDificultad, name="nivel_dificultad", create_type=False),
        nullable=True
    )
    tipo_curso = Column(
        ENUM(TipoCurso, name="tipo_curso", create_type=False),
        nullable=True
    )
    categoria_curso = Column(
        ENUM(CategoriaCurso, name="categoria_curso", create_type=False),
        nullable=True
    )
    estado = Column(
    ...
    idioma = Column(
        ENUM(IdiomaCurso, name="idioma", create_type=False),
        nullable=True
    )
    ...
    correquisitos_ids = Column(JSON)  # Lista de curso_ids que deben cursarse simultáneamente
