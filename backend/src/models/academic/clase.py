...
from sqlalchemy.dialects.postgresql import ENUM, INTEGER, TEXT, TIMESTAMP, UUID
...
class Clase(Base):
    ...
    material_clases = relationship(
        "MaterialClase", backref="clase", cascade="all, delete-orphan"
    )
    historial_accesos = relationship(
        "HistorialAccesoClase", backref="clase", cascade="all, delete-orphan"
    )
    ...
    def codigo_activo(self) -> bool:
        ...
        if self.estado_codigo != EstadoCodigoAcceso.activo:
            return False
        return not (
            self.fecha_vencimiento_codigo and func.now() > self.fecha_vencimiento_codigo
        )
