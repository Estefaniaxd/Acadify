...
try:
    ...
    print(f"Warning: Failed to connect to Redis for avatar service: {e}")
...
async def get_user_avatars(
    ...
    """
    # Verificar permisos
    if (current_user.usuario_id != user_id and 
        str(current_user.rol) != 'administrador'):
        raise HTTPException(
    ...
    try:
        ...
        return UserAvatarListResponse(
            ...
            active_avatar_id=str(stats['active_avatar_id']) if stats['active_avatar_id'] else None
        )
        
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error fetching user avatars: {str(e)}"
        )
...
async def cleanup_temp_previews(
    ...
    try:
        ...
        background_tasks.add_task(
            avatar_service.cleanup_temp_previews,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error scheduling cleanup: {str(e)}"
        )